...
func seedFlowQuotaData(t *testing.T, quotaData QuotaData) {
	t.Helper()
	require.NoError(t, dbx.DB.Create(&quotaData).Error)
}

func seedFlowLookupData(t *testing.T) {
	t.Helper()
	require.NoError(t, dbx.DB.Create(&Channel{Id: 1, Name: "east"}).Error)
	require.NoError(t, dbx.DB.Create(&Channel{Id: 2, Name: "west"}).Error)
	require.NoError(t, dbx.DB.Create(&Token{Id: 11, UserId: 1, Key: "sk-primary", Name: "primary"}).Error)
	require.NoError(t, dbx.DB.Create(&Token{Id: 22, UserId: 2, Key: "sk-backup", Name: "backup"}).Error)
	require.NoError(t, dbx.DB.Delete(&Token{Id: 11}).Error)
}
...
func TestLogQuotaDataSplitsRowsByUseGroupTokenChannelAndNode(t *testing.T) {
	...
	var rows []QuotaData
	require.NoError(t, dbx.DB.Order("quota DESC").Find(&rows).Error)
