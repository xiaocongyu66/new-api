...
func GetTwoFAByUserId(userId int) (*TwoFA, error) {
	...
	var twoFA TwoFA
	err := dbx.DB.Where("user_id = ?", userId).First(&twoFA).Error
	if err != nil {
...
func (t *TwoFA) CreatePendingTwoFASetup() error {
	...
	// 验证用户存在
	var user User
	if err := dbx.DB.First(&user, t.UserId).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
...
func (t *TwoFA) updateUsageState() error {
	...
	return dbx.DB.Model(&TwoFA{}).Where("id = ?", t.Id).Updates(map[string]interface{}{
...
func (t *TwoFA) IncrementFailedAttempts() error {
	...
	for range maxUpdateRetries {
		var current TwoFA
		if err := dbx.DB.Select("id", "failed_attempts", "locked_until").First(&current, t.Id).Error; err != nil {
			return err
		}
...
func CreatePendingTwoFASetupBackupCodes(userId int, codes []string) error {
	return dbx.DB.Transaction(func(tx *gorm.DB) error {
		var pending TwoFA
		if err := dbx.LockForUpdate(tx).Where("user_id = ? AND is_enabled = ?", userId, false).First(&pending).Error; err != nil {
			return err
		}
...
func ReplaceBackupCodesWithAuthVersion(userId int, codes []string) error {
	if err := dbx.DB.Transaction(func(tx *gorm.DB) error {
		var enabled TwoFA
		if err := dbx.LockForUpdate(tx).Where("user_id = ? AND is_enabled = ?", userId, true).First(&enabled).Error; err != nil {
			if errors.Is(err, gorm.ErrRecordNotFound) {
...
func ValidateBackupCode(userId int, code string) (bool, error) {
	...
	if err := dbx.DB.Where("user_id = ? AND is_used = false", userId).Find(&backupCodes).Error; err != nil {
		return false, err
	}
	...
	for _, bc := range backupCodes {
		if common.ValidatePasswordAndHash(normalizedCode, bc.CodeHash) {
			now := time.Now()
			result := dbx.DB.Model(&TwoFABackupCode{}).
...
func GetUnusedBackupCodeCount(userId int) (int, error) {
	var count int64
	err := dbx.DB.Model(&TwoFABackupCode{}).Where("user_id = ? AND is_used = false", userId).Count(&count).Error
...
func DisableTwoFAWithAuthVersion(userId int) error {
	if err := dbx.DB.Transaction(func(tx *gorm.DB) error {
		var twoFA TwoFA
		if err := dbx.LockForUpdate(tx).Where("user_id = ? AND is_enabled = ?", userId, true).First(&twoFA).Error; err != nil {
			if errors.Is(err, gorm.ErrRecordNotFound) {
				return ErrTwoFANotEnabled
			}
			return err
...
func (t *TwoFA) EnableWithAuthVersion() error {
	...
	if err := dbx.DB.Transaction(func(tx *gorm.DB) error {
		var pending TwoFA
		if err := dbx.LockForUpdate(tx).Where("id = ? AND user_id = ? AND is_enabled = ?", t.Id, t.UserId, false).First(&pending).Error; err != nil {
			if errors.Is(err, gorm.ErrRecordNotFound) {
...
func GetTwoFAStats() (map[string]interface{}, error) {
	...
	// 总用户数
	if err := dbx.DB.Model(&User{}).Count(&totalUsers).Error; err != nil {
		return nil, err
	}

	// 启用2FA的用户数
	if err := dbx.DB.Model(&TwoFA{}).Where("is_enabled = true").Count(&enabledUsers).Error; err != nil {
		return nil, err
	}
