...
// SensitiveOutputState 是请求级输出过滤状态（挂在 context）。
...
var OutputHoldbackLen = sync.OnceValue(func() int {
	...
	for _, d := range sensitive.TargetDomains() {
...
func OutputChunkFiltered(c contract.Context, data string) string {
	...
	if sensitive.ShouldCheckCompletionSensitive() {
		cleaned, labels, cuts = sensitive.SanitizeSensitiveTextRanges(scan)
	} else {
		cleaned, labels, cuts = sensitive.SanitizeTargetDomainsRanges(scan)
	}
	if len(labels) > 0 {
		common.SysLog("output sanitized by sensitive filter: [" + strings.Join(labels, ",") + "]")
		sensitive.RecordSensitiveBlock(c, "output", "sanitize:"+strings.Join(labels, ","), scan)
	}
