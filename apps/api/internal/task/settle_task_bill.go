// Package task holds async task capabilities including polling, storage, and billing settlement.
package task
...
func taskAdjustFunding(task *Task, delta int) error {
	if taskIsSubscription(task) {
		return dbinfra.PostConsumeUserSubscriptionDelta(task.PrivateData.SubscriptionId, int64(delta))
	}
	if delta > 0 {
...
func AttachQuotaSaturation(c contract.Context, relayInfo *relaycommon.RelayInfo, other map[string]interface{}) {
	if relayInfo == nil {
		return
	}
	...
	AttachQuotaSaturationToOther(other, clamp)
	logger.LogWarn(c.Context(), fmt.Sprintf("quota saturation on consume log: op=%s kind=%s original=%g clamped=%d user=%d model=%s",
		clamp.Op, clamp.Kind, clamp.Original, clamp.Clamped, relayInfo.UserId, relayInfo.OriginModelName))
}

// AttachQuotaSaturationToOther 将配额饱和信息写入 other["admin_info"]，仅管理员可见。
