...
func RunMidjourneyTaskUpdateOnce(ctx context.Context, report func(processed, total int)) MidjourneyPollSummary {
	...
	tasks := taskmodel.GetAllUnFinishTasks()
	if len(tasks) == 0 {
		return summary
	}
	...
	taskM := make(map[string]*taskmodel.Midjourney)
	...
	if len(nullTaskIds) > 0 {
		...
		err := taskmodel.MjBulkUpdateByTaskIds(nullTaskIds, map[string]any{
			"status":   "FAILURE",
			"progress": "100%",
		})
		if err != nil {
	...
	for channelId, taskIds := range taskChannelM {
		...
		midjourneyChannel, err := catalogmodel.CacheGetChannel(channelId)
		if err != nil {
			logger.LogError(ctx, fmt.Sprintf("CacheGetChannel: %v", err))
			err := taskmodel.MjBulkUpdate(taskIds, map[string]any{
		...
		var responseItems []taskdto.MidjourneyDto
		...
		for _, responseItem := range responseItems {
			...
			if err != nil {
				...
				RefundMidjourneyQuota(ctx, task, "构图失败")
			}
		}
	}
...
func checkMjTaskNeedUpdate(oldTask *taskmodel.Midjourney, newTask taskdto.MidjourneyDto) bool {
	if oldTask.Code != 1 {
		return true
	}
