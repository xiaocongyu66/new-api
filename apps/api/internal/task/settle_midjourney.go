...
func PrepareMidjourneyTaskBilling(relayInfo *relaycommon.RelayInfo, task *Midjourney, quota int, shouldBill bool) (bool, error) {
	if task == nil {
...
func SettleMidjourneyTaskBilling(relayInfo *relaycommon.RelayInfo, task *Midjourney, prepared bool) (bool, error) {
	if !prepared {
		return false, nil
	}
	if relayInfo == nil {
...
func RefundMidjourneyQuota(ctx context.Context, task *Midjourney, reason string) bool {
	quota := task.Quota
	if quota == 0 {
		return true
	}
...
// PostConsumeQuotaWithResult wraps settlecore.PostConsumeQuotaWithResult with the
// 4-arg shape the legacy controller callers still expect (sendEmail flag).
func PostConsumeQuotaWithResult(relayInfo *relaycommon.RelayInfo, quota int, preConsumedQuota int, sendEmail bool) (settlecore.PostConsumeQuotaResult, error) {
	_ = sendEmail
	return settlecore.PostConsumeQuotaWithResult(relayInfo, quota, preConsumedQuota)
}
