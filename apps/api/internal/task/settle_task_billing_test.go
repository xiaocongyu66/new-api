...
func TestTaskBillingOtherFiltersHistoricalOtherRatios(t *testing.T) {
	task := makeTask(1, 1, 100, 0, settlecore.BillingSourceWallet, 0)
...
func TestTaskBillingContextPriceDataFiltersMultiplier(t *testing.T) {
	priceData := taskBillingContextPriceData(&TaskBillingContext{
...
func getSubscriptionUsed(t *testing.T, id int) int64 {
	t.Helper()
	var sub billing.UserSubscription
...
func TestRefundTaskQuota_ZeroQuota(t *testing.T) {
	...
	task := makeTask(userID, 0, 0, 0, settlecore.BillingSourceWallet, 0)
...
func TestRecalculate_PositiveDelta(t *testing.T) {
	...
	log := getLastLog(t)
	require.NotNil(t, log)
	assert.Equal(t, usage.LogTypeConsume, log.Type)
...
func TestRecalculate_ZeroDelta(t *testing.T) {
	...
	task := makeTask(userID, 0, preConsumed, 0, settlecore.BillingSourceWallet, 0)
...
func TestRecalculate_ActualQuotaZero(t *testing.T) {
	...
	task := makeTask(userID, 0, 5000, 0, settlecore.BillingSourceWallet, 0)
...
func simulatePollBilling(ctx context.Context, task *Task, newStatus TaskStatus, actualQuota int) {
	snap := task.Snapshot()
	...
	isDone := task.Status == TaskStatus(TaskStatusSuccess) || task.Status == TaskStatus(TaskStatusFailure)
...
func TestCASGuardedRefund_Win(t *testing.T) {
	...
	task := makeTask(userID, channelID, preConsumed, tokenID, settlecore.BillingSourceWallet, 0)
	task.Status = TaskStatus(TaskStatusInProgress)
	require.NoError(t, dbx.DB.Create(task).Error)

	simulatePollBilling(ctx, task, TaskStatus(TaskStatusFailure), 0)
...
func TestCASGuardedRefund_Lose(t *testing.T) {
	...
	dbx.DB.Model(&Task{}).Where("id = ?", task.ID).Update("status", TaskStatusFailure)
...
func TestCASGuardedSettle_Win(t *testing.T) {
	...
	simulatePollBilling(ctx, task, TaskStatus(TaskStatusSuccess), actualQuota)
...
func TestNonTerminalUpdate_NoBilling(t *testing.T) {
	...
	simulatePollBilling(ctx, task, TaskStatus(TaskStatusInProgress), 0)
...
func (m *mockAdaptor) AdjustBillingOnComplete(_ *Task, _ *relaycommon.TaskInfo) int {
...
func TestSettle_PerCallBilling_SkipsAdaptorAdjust(t *testing.T) {
	...
	taskResult := &relaycommon.TaskInfo{Status: TaskStatusSuccess}
...
func TestSettle_PerCallBilling_SkipsTotalTokens(t *testing.T) {
	...
	taskResult := &relaycommon.TaskInfo{Status: TaskStatusSuccess, TotalTokens: 9999}
