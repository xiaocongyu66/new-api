...
func TestIsClickHouseDSN(t *testing.T) {
	...
	for _, c := range cases {
		assert.Equalf(t, c.want, common.IsClickHouseDSN(c.dsn), "dsn=%q", c.dsn)
	}
}

func TestNormalizeClickHouseDSN(t *testing.T) {
	// https without secure gets secure=true appended
	normalized := common.NormalizeClickHouseDSN("https://default:pass@localhost:8443/logs")
	...
	assert.Equal(t,
		...
		common.NormalizeClickHouseDSN("https://localhost:8443/logs?secure=false"),
	)

	// non-https schemes are returned verbatim
	assert.Equal(t, "clickhouse://localhost:9000/logs", common.NormalizeClickHouseDSN("clickhouse://localhost:9000/logs"))
	assert.Equal(t, "tcp://localhost:9000/logs", common.NormalizeClickHouseDSN("tcp://localhost:9000/logs"))
}
