...
func TestTokenUpdateSynchronouslyNarrowsPreheatedAutoGroupsCache(t *testing.T) {
	...
	preheated, err := identity.CacheGetTokenByKey(token.Key)
	require.NoError(t, err)
	...
	_, cacheErr := identity.CacheGetTokenByKey(token.Key)
	require.Error(t, cacheErr, "the pre-update cache entry must be invalidated")
	reloaded, err := identity.GetTokenByKey(token.Key, false)
	require.NoError(t, err)
...
// 模拟“已水合”的缓存状态。
func cacheSetTokenForTest(token identity.Token) error {
	_, err := identity.CacheInitToken(token)
	return err
}
