...
func seedFlowLookupData(t *testing.T) {
	t.Helper()
	require.NoError(t, dbx.DB.Create(&catalog.Channel{Id: 1, Name: "east"}).Error)
	require.NoError(t, dbx.DB.Create(&catalog.Channel{Id: 2, Name: "west"}).Error)
	...
	require.NoError(t, dbx.DB.Delete(&identity.Token{Id: 11}).Error)
}
