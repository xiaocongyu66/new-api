...
func RecordLog(userId int, logType int, content string) {
	...
	username, _ := identity.GetUsernameById(userId, false)
	log := &Log{
		UserId:    userId,
		Username:  username,
...
func RecordOperationAuditLog(logUserId int, content string, ip string, action string, params map[string]interface{}, adminInfo map[string]interface{}, auditInfo map[string]interface{}) {
	username, _ := identity.GetUsernameById(logUserId, false)
	other := map[string]interface{}{
...
func RecordSensitiveAuditLog(userId int, content string, ip string, params map[string]interface{}) {
	...
	if userId > 0 && dbx.DB != nil {
		username, _ = identity.GetUsernameById(userId, true)
	}
	other := map[string]interface{}{
...
func RecordErrorLog(c contract.Context, userId int, channelId int, modelName string, tokenName string, content string, tokenId int, useTimeSeconds int,
	...
	// 判断是否需要记录 IP
	needRecordIp := false
	if settingMap, err := identity.GetUserSetting(userId, false); err == nil {
...
func RecordConsumeLog(c contract.Context, userId int, params RecordConsumeLogParams) {
...
func RecordTaskBillingLog(params RecordTaskBillingLogParams) {
	...
	username, _ := identity.GetUsernameById(params.UserId, false)
	tokenName := ""
	if params.TokenId > 0 {
		if token, err := identity.GetTokenById(params.TokenId); err == nil {
	...
	if channelIds.Len() > 0 {
		...
		if common.MemoryCacheEnabled {
			...
			for _, channelId := range channelIds.Items() {
				if cacheChannel, err := catalog.BridgeCacheGetChannel(channelId); err == nil {
...
func GetLogByTokenIdInternal(tokenId int) (logs []*Log, err error) {
	order := "id desc"
	if common.UsingLogDatabase(common.DatabaseTypeClickHouse) {
		order = clickHouseLogOrder("")
	}
...
// GetAllLogsInternal mirrors GetAllLogs
func GetAllLogsInternal(logType int, startTimestamp int64, endTimestamp int64, modelName string, username string, tokenName string, startIdx int, num int, channel int, group string, requestId string, upstreamRequestId string) (logs []*Log, total int64, err error) {
	var tx *gorm.DB
	if logType == LogTypeUnknown {
		tx = dbx.LogDB
	} else {
...
// GetUserLogsInternal mirrors GetUserLogs
func GetUserLogsInternal(userId int, logType int, startTimestamp int64, endTimestamp int64, modelName string, tokenName string, startIdx int, num int, group string, requestId string, upstreamRequestId string) (logs []*Log, total int64, err error) {
	var tx *gorm.DB
	if logType == LogTypeUnknown {
		tx = dbx.LogDB.Where("logs.user_id = ?", userId)
	} else {
...
// SumUsedQuotaInternal mirrors SumUsedQuota
func SumUsedQuotaInternal(logType int, startTimestamp int64, endTimestamp int64, modelName string, username string, tokenName string, channel int, group string) (stat Stat, err error) {
	tx := dbx.LogDB.Table("logs").Select("COALESCE(sum(quota), 0) quota")
...
// SumUsedTokenInternal mirrors SumUsedToken
