...
func fillFlowTokenNames(rows []*FlowQuotaData) error {
	...
	if err := identity.TokenQuery(dbx.DB).Select("id, name").Where("id IN ?", tokenIDs).Find(&tokens).Error; err != nil {
		return err
	}
...
func fillFlowChannelNames(rows []*FlowQuotaData) error {
	...
	if common.MemoryCacheEnabled {
		for _, channelID := range channelIDs {
			if cachedChannel, err := channel.BridgeCacheGetChannel(channelID); err == nil {
				channelNameByID[channelID] = cachedChannel.Name
			}
		}
	} else {
		var channels []struct {
...
func GetAllQuotaDatesInternal(startTime int64, endTime int64, username string) (quotaData []*QuotaData, err error) {
	if username != "" {
