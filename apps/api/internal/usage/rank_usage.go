...
func buildRankingsSnapshot(config rankingPeriodConfig, now time.Time) (*RankingsResponse, error) {
	...
	currentTotals, err := GetRankingQuotaTotals(startTime, endTime)
	if err != nil {
		return nil, err
	}
	currentBuckets, err := GetRankingQuotaBuckets(startTime, endTime, config.bucketSize)
	if err != nil {
		return nil, err
	}

	var previousTotals []RankingQuotaTotal
	if config.hasPrevious {
		previousStart, previousEnd := previousRankingTimeRange(config, startTime)
		previousTotals, err = GetRankingQuotaTotals(previousStart, previousEnd)
		if err != nil {
			return nil, err
		}
	}
...
func buildRankingModelMeta() map[string]rankingModelMeta {
	vendorByID := make(map[int]channel.PricingVendor)
	for _, vendor := range channel.GetVendors() {
		vendorByID[vendor.ID] = vendor
	}

	meta := make(map[string]rankingModelMeta)
	for _, pricing := range channel.GetPricing() {
...
func buildRankedModels(totals []RankingQuotaTotal, totalTokens int64, previousRanks map[string]int, previousTokens map[string]int64, meta map[string]rankingModelMeta, showGrowth bool) []RankedModel {
...
func buildRankedVendors(currentTotals []RankingQuotaTotal, previousTotals []RankingQuotaTotal, totalTokens int64, meta map[string]rankingModelMeta, showGrowth bool) []RankedVendor {
...
func buildModelHistory(buckets []RankingQuotaBucket, totals []RankingQuotaTotal, meta map[string]rankingModelMeta, config rankingPeriodConfig) ModelHistorySeries {
...
func buildVendorShareHistory(buckets []RankingQuotaBucket, vendors []RankedVendor, totalTokens int64, meta map[string]rankingModelMeta, config rankingPeriodConfig) VendorShareSeries {
...
func rankingRankMap(totals []RankingQuotaTotal) map[string]int {
...
func rankingTokenMap(totals []RankingQuotaTotal) map[string]int64 {
...
func sumRankingTokens(totals []RankingQuotaTotal) int64 {
	total := int64(0)
	for _, item := range totals {
