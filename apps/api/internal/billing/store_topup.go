...
func ValidateTopUpQuotaCapacity(userId int, creditedQuota int) error {
	...
	user, err := identity.ReadUserQuota(dbx.DB, userId)
	if err != nil {
		return err
	}
...
func creditTopUpQuota(tx *gorm.DB, userId int, creditedQuota int, updates map[string]interface{}) error {
	...
	var count int64
	if err := identity.UserQuery(tx).Where("id = ?", userId).Count(&count).Error; err != nil {
		return err
	}
	if count == 0 {
...
func RechargeEpay(tradeNo string, actualPaymentMethod string, callerIp string) (alreadyDone bool, err error) {
	...
	quotacache.SyncCredit(topUp.UserId, quotaToAdd, "epay topup")
	...
	usage.RecordTopupLog(topUp.UserId, fmt.Sprintf("使用在线充值成功，充值金额: %v，支付金额：%f", logger.LogQuota(quotaToAdd), topUp.Money), callerIp, topUp.PaymentMethod, PaymentProviderEpay)
	return false, nil
}
...
func QueryAllTopUps(pageInfo *common.PageInfo) (topups []*TopUp, total int64, err error) {
	tx := dbx.DB.Begin()
	if tx.Error != nil {
...
func ManualCompleteTopUp(tradeNo string, callerIp string) error {
	...
	// 事务外记录日志，避免阻塞
	quotacache.SyncCredit(userId, quotaToAdd, "manual topup")
	usage.RecordTopupLog(userId, fmt.Sprintf("管理员补单成功，充值金额: %v，支付金额：%f", logger.FormatQuota(quotaToAdd), payMoney), callerIp, paymentMethod, "admin")
	return nil
}
func RechargeWaffoPancake(tradeNo string) (err error) {
	...
	quotacache.SyncCredit(topUp.UserId, quotaToAdd, "waffo pancake topup")

	if quotaToAdd > 0 {
		usage.RecordLog(topUp.UserId, usage.LogTypeTopup, fmt.Sprintf("Waffo Pancake充值成功，充值额度: %v，支付金额: %.2f", logger.FormatQuota(quotaToAdd), topUp.Money))
	}
...
func RechargeWaffo(tradeNo string, callerIp string) (err error) {
	...
	quotacache.SyncCredit(topUp.UserId, quotaToAdd, "waffo topup")

	if quotaToAdd > 0 {
		usage.RecordTopupLog(topUp.UserId, fmt.Sprintf("Waffo充值成功，充值额度: %v，支付金额: %.2f", logger.FormatQuota(quotaToAdd), topUp.Money), callerIp, topUp.PaymentMethod, PaymentMethodWaffo)
	}
...
func RechargeCreem(referenceId string, customerEmail string, customerName string, callerIp string) (err error) {
	...
	err = dbx.DB.Transaction(func(tx *gorm.DB) error {
		...
		if customerEmail != "" {
			// 先检查用户当前邮箱是否为空
			user, err := identity.LockUserRow(tx, topUp.UserId)
			if err != nil {
				return err
			}
	...
	quotacache.SyncCredit(topUp.UserId, quota, "creem topup")

	usage.RecordTopupLog(topUp.UserId, fmt.Sprintf("使用Creem充值成功，充值额度: %v，支付金额：%.2f", quota, topUp.Money), callerIp, topUp.PaymentMethod, PaymentMethodCreem)

	return nil
}
func QueryUserTopUps(userId int, pageInfo *common.PageInfo) (topups []*TopUp, total int64, err error) {
	// Start transaction
	tx := dbx.DB.Begin()
	if tx.Error != nil {
		return nil, 0, tx.Error
...
func Recharge(referenceId string, customerId string, callerIp string) (err error) {
	...
	usage.RecordTopupLog(topUp.UserId, fmt.Sprintf("使用在线充值成功，充值金额: %v，支付金额：%d", logger.FormatQuota(quota), topUp.Amount), callerIp, topUp.PaymentMethod, PaymentMethodStripe)
