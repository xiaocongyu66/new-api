...
func TestClaude_StandardTier(t *testing.T) {
	cost, trace, err := priceexpr.RunExpr(claudeExpr, priceexpr.TokenParams{P: 100000, C: 5000})
	if err != nil {
		t.Fatal(err)
	}
...
func TestClaude_LongContextTier(t *testing.T) {
	cost, trace, err := priceexpr.RunExpr(claudeExpr, priceexpr.TokenParams{P: 300000, C: 10000})
	if err != nil {
		t.Fatal(err)
	}
...
func TestClaude_BoundaryExact(t *testing.T) {
	cost, trace, err := priceexpr.RunExpr(claudeExpr, priceexpr.TokenParams{P: 200000, C: 1000})
	if err != nil {
		t.Fatal(err)
	}
...
func TestGLM_Tier1(t *testing.T) {
	cost, trace, err := priceexpr.RunExpr(glmExpr, priceexpr.TokenParams{P: 15000, C: 100})
	if err != nil {
		t.Fatal(err)
	}
...
func TestGLM_Tier2(t *testing.T) {
	cost, trace, err := priceexpr.RunExpr(glmExpr, priceexpr.TokenParams{P: 15000, C: 500})
	if err != nil {
		t.Fatal(err)
	}
...
func TestGLM_Tier3(t *testing.T) {
	cost, trace, err := priceexpr.RunExpr(glmExpr, priceexpr.TokenParams{P: 50000, C: 100})
	if err != nil {
		t.Fatal(err)
	}
...
func TestSimpleExpr_NoTier(t *testing.T) {
	cost, trace, err := priceexpr.RunExpr("p * 0.5 + c * 1.0", priceexpr.TokenParams{P: 1000, C: 500})
	if err != nil {
		t.Fatal(err)
	}
...
func TestMathHelpers(t *testing.T) {
	cost, _, err := priceexpr.RunExpr("max(p, c) * 0.5 + min(p, c) * 0.1", priceexpr.TokenParams{P: 300, C: 500})
	if err != nil {
		t.Fatal(err)
	}
...
func TestRequestProbeHelpers(t *testing.T) {
	cost, _, err := priceexpr.RunExprWithRequest(
		`p * 0.5 + c * 1.0 * (param("service_tier") == "fast" ? 2 : 1)`,
		priceexpr.TokenParams{P: 1000, C: 500},
		priceexpr.RequestInput{
...
func TestRequestProbeMultipleRulesTraceAllFactors(t *testing.T) {
	...
	cost, trace, err := priceexpr.RunExprWithRequest(
		exprStr,
		priceexpr.TokenParams{P: 10},
		priceexpr.RequestInput{
			Headers: map[string]string{
				"Anthropic-Beta": "fast-mode-2026-02-01",
			},
	...
	assert.Equal(t, []priceexpr.RequestRuleTrace{
...
func TestRequestProbeTraceIncludesUnmatchedFactors(t *testing.T) {
	...
	cost, trace, err := priceexpr.RunExprWithRequest(
		exprStr,
		priceexpr.TokenParams{P: 10},
		priceexpr.RequestInput{Body: []byte(`{"service_tier":"fast"}`)},
	)
...
func TestRequestProbeNonUnitFallbackIsNotTraced(t *testing.T) {
	cost, trace, err := priceexpr.RunExprWithRequest(
		...
		priceexpr.TokenParams{},
		priceexpr.RequestInput{Body: []byte(`{"service_tier":"standard"}`)},
	)
...
func TestRequestProbeInternalTraceFunctionIsReserved(t *testing.T) {
	_, err := priceexpr.CompileFromCache(`_trace(0, true, 5.0)`)

	require.ErrorContains(t, err, `identifier "_trace" is reserved for internal use`)
}

func TestCeilFloor(t *testing.T) {
	cost, _, err := priceexpr.RunExpr("ceil(p / 1000) * 0.5", priceexpr.TokenParams{P: 1500})
	if err != nil {
		t.Fatal(err)
	}
...
func TestZeroTokens(t *testing.T) {
	cost, _, err := priceexpr.RunExpr(claudeExpr, priceexpr.TokenParams{})
	if err != nil {
		t.Fatal(err)
	}
	if cost != 0 {
...
func TestQuotaRound(t *testing.T) {
	...
	for _, tt := range tests {
		got := common.QuotaRound(tt.in)
		if got != tt.want {
			t.Errorf("common.QuotaRound(%f) = %d, want %d", tt.in, got, tt.want)
		}
	}
}
...
func TestComputeTieredQuota_Basic(t *testing.T) {
	snap := &priceexpr.BillingSnapshot{
		...
		ExprHash:                  priceexpr.ExprHashString(claudeExpr),
		GroupRatio:                1.0,
		...
		EstimatedQuotaAfterGroup:  common.QuotaRound((100000*1.5 + 5000*7.5) / 1_000_000 * 500_000),
		EstimatedTier:             "standard",
		QuotaPerUnit:              500_000,
	}

	result, err := priceexpr.ComputeTieredQuota(snap, priceexpr.TokenParams{P: 300000, C: 10000})
	if err != nil {
		t.Fatal(err)
	}
...
func TestComputeTieredQuota_SameTier(t *testing.T) {
	snap := &priceexpr.BillingSnapshot{
		...
		EstimatedQuotaAfterGroup:  common.QuotaRound((50000*1.5 + 1000*7.5) / 1_000_000 * 500_000 * 1.5),
		EstimatedTier:             "standard",
		QuotaPerUnit:              500_000,
	}

	result, err := priceexpr.ComputeTieredQuota(snap, priceexpr.TokenParams{P: 80000, C: 2000})
	if err != nil {
		t.Fatal(err)
	}

	wantBefore := (80000*1.5 + 2000*7.5) / 1_000_000 * 500_000
	wantAfter := common.QuotaRound(wantBefore * 1.5)
...
func TestCompileError(t *testing.T) {
	_, _, err := priceexpr.RunExpr("invalid +-+ syntax", priceexpr.TokenParams{})
	if err == nil {
...
func TestCompileCache_SameResult(t *testing.T) {
	r1, _, err := priceexpr.RunExpr("p * 0.5", priceexpr.TokenParams{P: 100})
	if err != nil {
		t.Fatal(err)
	}
	r2, _, err := priceexpr.RunExpr("p * 0.5", priceexpr.TokenParams{P: 100})
	if err != nil {
		t.Fatal(err)
	}
	if r1 != r2 {
...
func TestInvalidateCache(t *testing.T) {
	priceexpr.InvalidateCache()
	r1, _, _ := priceexpr.RunExpr("p * 0.5", priceexpr.TokenParams{P: 100})
	priceexpr.InvalidateCache()
	r2, _, _ := priceexpr.RunExpr("p * 0.5", priceexpr.TokenParams{P: 100})
	if r1 != r2 {
...
func TestExprHashString_Deterministic(t *testing.T) {
	h1 := priceexpr.ExprHashString("p * 0.5")
	h2 := priceexpr.ExprHashString("p * 0.5")
	if h1 != h2 {
		t.Error("hash should be deterministic")
	}
	h3 := priceexpr.ExprHashString("p * 0.6")
	if h1 == h3 {
...
func TestCachePresent_StandardTier(t *testing.T) {
	params := priceexpr.TokenParams{P: 100000, C: 5000, CR: 50000, CC: 10000}
	cost, trace, err := priceexpr.RunExpr(claudeWithCacheExpr, params)
	if err != nil {
		t.Fatal(err)
	}
...
func TestCachePresent_LongContextTier(t *testing.T) {
	params := priceexpr.TokenParams{P: 300000, C: 10000, CR: 100000, CC: 20000}
	cost, trace, err := priceexpr.RunExpr(claudeWithCacheExpr, params)
	if err != nil {
		t.Fatal(err)
	}
...
func TestCacheAbsent_ZeroCacheTokens(t *testing.T) {
	params := priceexpr.TokenParams{P: 100000, C: 5000}
	cost, trace, err := priceexpr.RunExpr(claudeWithCacheExpr, params)
	if err != nil {
		t.Fatal(err)
	}
...
func TestMixedCacheFields(t *testing.T) {
	params := priceexpr.TokenParams{P: 100000, C: 5000, CR: 10000, CC: 5000, CC1h: 2000}
	cost, _, err := priceexpr.RunExpr(claudeCacheSplitExpr, params)
	if err != nil {
		t.Fatal(err)
	}
...
// ---------------------------------------------------------------------------
// Backward compatibility: p+c only expressions still work with priceexpr.TokenParams
// ---------------------------------------------------------------------------

func TestBackwardCompat_OldExprWithTokenParams(t *testing.T) {
	params := priceexpr.TokenParams{P: 100000, C: 5000, CR: 99999, CC: 88888}
	cost, trace, err := priceexpr.RunExpr(claudeExpr, params)
	if err != nil {
		t.Fatal(err)
	}
	want := 100000*1.5 + 5000*7.5
...
func TestComputeTieredQuota_WithCache(t *testing.T) {
	snap := &priceexpr.BillingSnapshot{
		...
		ExprHash:                  priceexpr.ExprHashString(claudeWithCacheExpr),
		GroupRatio:                1.0,
		EstimatedPromptTokens:     100000,
		EstimatedCompletionTokens: 5000,
		EstimatedQuotaBeforeGroup: (100000*1.5 + 5000*7.5) / 1_000_000 * 500_000,
	...
	params := priceexpr.TokenParams{P: 100000, C: 5000, CR: 50000, CC: 10000}
	result, err := priceexpr.ComputeTieredQuota(snap, params)
	if err != nil {
		t.Fatal(err)
	}
...
func TestComputeTieredQuota_WithCacheCrossTier(t *testing.T) {
	snap := &priceexpr.BillingSnapshot{
		...
		EstimatedCompletionTokens: 5000,
		EstimatedQuotaBeforeGroup: (100000*1.5 + 5000*7.5) / 1_000_000 * 500_000,
		EstimatedQuotaAfterGroup:  common.QuotaRound((100000*1.5 + 5000*7.5) / 1_000_000 * 500_000 * 2.0),
		EstimatedTier:             "standard",
		QuotaPerUnit:              500_000,
	}

	params := priceexpr.TokenParams{P: 300000, C: 10000, CR: 50000, CC: 10000}
	result, err := priceexpr.ComputeTieredQuota(snap, params)
	if err != nil {
		t.Fatal(err)
	}
	...
	wantAfter := common.QuotaRound(wantBefore * 2.0)
	if math.Abs(result.ActualQuotaBeforeGroup-wantBefore) > 1e-6 {
		t.Errorf("before group: got %f, want %f", result.ActualQuotaBeforeGroup, wantBefore)
	}
	if result.ActualQuotaAfterGroup != wantAfter {
...
func TestComputeTieredQuota_BasicSettlement(t *testing.T) {
	...
	result, err := priceexpr.ComputeTieredQuota(snap, priceexpr.TokenParams{P: 3000, C: 2000})
	if err != nil {
		t.Fatal(err)
	}
...
func TestComputeTieredQuota_ZeroTokens(t *testing.T) {
	...
	result, err := priceexpr.ComputeTieredQuota(snap, priceexpr.TokenParams{})
	if err != nil {
		t.Fatal(err)
	}
...
func TestComputeTieredQuota_RoundingEdge(t *testing.T) {
	...
	result, err := priceexpr.ComputeTieredQuota(snap, priceexpr.TokenParams{P: 3})
	if err != nil {
		t.Fatal(err)
	}
...
func TestComputeTieredQuotaWithRequest_ProbeAffectsQuota(t *testing.T) {
	...
	r1, err := priceexpr.ComputeTieredQuota(snap, priceexpr.TokenParams{P: 1000})
	if err != nil {
		t.Fatal(err)
	}
	...
	r2, err := priceexpr.ComputeTieredQuotaWithRequest(snap, priceexpr.TokenParams{P: 1000}, priceexpr.RequestInput{
		Body: []byte(`{"fast":true}`),
	})
	if err != nil {
		t.Fatal(err)
...
func TestComputeTieredQuota_BoundaryTierCrossing(t *testing.T) {
	...
	r1, err := priceexpr.ComputeTieredQuota(snap, priceexpr.TokenParams{P: 100000})
	if err != nil {
		t.Fatal(err)
	}
	...
	r2, err := priceexpr.ComputeTieredQuota(snap, priceexpr.TokenParams{P: 100001})
	if err != nil {
		t.Fatal(err)
	}
...
func TestTimeFunctions_ValidTimezone(t *testing.T) {
	...
	cost, _, err := priceexpr.RunExpr(exprStr, priceexpr.TokenParams{P: 100})
	if err != nil {
		t.Fatal(err)
	}
	if cost != 100 {
...
func TestTimeFunctions_AllFunctionsCompile(t *testing.T) {
	...
	cost, _, err := priceexpr.RunExpr(exprStr, priceexpr.TokenParams{P: 500})
	if err != nil {
		t.Fatal(err)
	}
	if cost != 500 {
...
func TestTimeFunctions_NightDiscountPattern(t *testing.T) {
	...
	cost, _, err := priceexpr.RunExpr(exprStr, priceexpr.TokenParams{P: 1000, C: 500})
	if err != nil {
		t.Fatal(err)
	}
...
func TestTimeFunctions_MonthDayPattern(t *testing.T) {
	...
	cost, _, err := priceexpr.RunExpr(exprStr, priceexpr.TokenParams{P: 1000})
	if err != nil {
		t.Fatal(err)
	}
...
func TestImageTokenVariable(t *testing.T) {
	...
	cost, _, err := priceexpr.RunExpr(exprStr, priceexpr.TokenParams{P: 1000, C: 500, Img: 200})
	if err != nil {
		t.Fatal(err)
	}
...
func TestAudioTokenVariables(t *testing.T) {
	...
	cost, _, err := priceexpr.RunExpr(exprStr, priceexpr.TokenParams{P: 1000, C: 500, AI: 100, AO: 50})
	if err != nil {
		t.Fatal(err)
	}
...
func TestImageAudioVariables(t *testing.T) {
	...
	cost, _, err := priceexpr.RunExpr(exprStr, priceexpr.TokenParams{P: 100, Img: 50, AI: 20, AO: 10})
	if err != nil {
		t.Fatal(err)
	}
...
func TestLen_StandardTier(t *testing.T) {
	params := priceexpr.TokenParams{P: 80000, C: 5000, Len: 100000, CR: 20000}
	cost, trace, err := priceexpr.RunExpr(lenTieredExpr, params)
	if err != nil {
		t.Fatal(err)
	}
...
func TestLen_LongContextTier(t *testing.T) {
	...
	params := priceexpr.TokenParams{P: 50000, C: 5000, Len: 300000, CR: 250000}
	cost, trace, err := priceexpr.RunExpr(lenTieredExpr, params)
	if err != nil {
		t.Fatal(err)
	}
...
func TestLen_BoundaryExact(t *testing.T) {
	params := priceexpr.TokenParams{P: 100000, C: 1000, Len: 200000, CR: 100000}
	_, trace, err := priceexpr.RunExpr(lenTieredExpr, params)
	if err != nil {
		t.Fatal(err)
	}
	if trace.MatchedTier != "standard" {
...
func TestLen_BoundaryPlusOne(t *testing.T) {
	params := priceexpr.TokenParams{P: 100000, C: 1000, Len: 200001, CR: 100001}
	_, trace, err := priceexpr.RunExpr(lenTieredExpr, params)
	if err != nil {
		t.Fatal(err)
	}
...
func TestLen_ZeroDefaultsToZero(t *testing.T) {
	...
	params := priceexpr.TokenParams{P: 1000, C: 500}
	_, trace, err := priceexpr.RunExpr(lenTieredExpr, params)
	if err != nil {
		t.Fatal(err)
	}
...
func BenchmarkExprCompile(b *testing.B) {
	for i := 0; i < b.N; i++ {
		priceexpr.InvalidateCache()
		priceexpr.CompileFromCache(benchComplexExpr)
	}
}

func BenchmarkExprRunCached(b *testing.B) {
	priceexpr.CompileFromCache(benchComplexExpr)
	params := priceexpr.TokenParams{P: 150000, C: 10000, Len: 188000, CR: 30000, CC: 5000, Img: 2000, AI: 1000, AO: 500}
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		priceexpr.RunExpr(benchComplexExpr, params)
	}
}
