...
func RequestWaffoAmount(c contract.Context) {
...
func RequestWaffoPay(c contract.Context) {
	...
	if req.PayMethodIndex != nil {
		...
		if idx < 0 || idx >= len(methods) {
			logger.LogWarn(c.Context(), fmt.Sprintf("Waffo 支付方式索引无效 user_id=%d pay_method_index=%d method_count=%d", id, idx, len(methods)))
			_ = c.JSON(http.StatusOK, common.H{"message": "error", "data": "不支持的支付方式"})
			return
		}
		...
		if !valid {
			logger.LogWarn(c.Context(), fmt.Sprintf("Waffo 支付方式无效 user_id=%d pay_method_type=%s pay_method_name=%q", id, req.PayMethodType, req.PayMethodName))
			_ = c.JSON(http.StatusOK, common.H{"message": "error", "data": "不支持的支付方式"})
			return
		}
	}
	...
	if err := topUp.Insert(); err != nil {
		logger.LogError(c.Context(), fmt.Sprintf("Waffo 创建充值订单失败 user_id=%d trade_no=%s amount=%d error=%q", id, merchantOrderId, req.Amount, err.Error()))
	...
	sdk, err := getWaffoSDK()
	if err != nil {
		logger.LogError(c.Context(), fmt.Sprintf("Waffo SDK 初始化失败 user_id=%d trade_no=%s error=%q", id, merchantOrderId, err.Error()))
		topUp.Status = common.TopUpStatusFailed
		_ = topUp.Update()
		_ = c.JSON(http.StatusOK, common.H{"message": "error", "data": "支付配置错误"})
		return
	}
	...
	resp, err := sdk.Order().Create(c.Context(), createParams, nil)
	if err != nil {
		logger.LogError(c.Context(), fmt.Sprintf("Waffo 创建订单失败 user_id=%d trade_no=%s error=%q", id, merchantOrderId, err.Error()))
		topUp.Status = common.TopUpStatusFailed
		_ = topUp.Update()
	...
	if !resp.IsSuccess() {
		logger.LogWarn(c.Context(), fmt.Sprintf("Waffo 创建订单业务失败 user_id=%d trade_no=%s code=%s message=%q response=%q", id, merchantOrderId, resp.Code, resp.Message, common.GetJsonString(resp)))
		topUp.Status = common.TopUpStatusFailed
		_ = topUp.Update()
		_ = c.JSON(http.StatusOK, common.H{"message": "error", "data": "拉起支付失败"})
		return
	...
	logger.LogInfo(c.Context(), fmt.Sprintf("Waffo 充值订单创建成功 user_id=%d trade_no=%s amount=%d money=%.2f pay_method_type=%s pay_method_name=%q", id, merchantOrderId, req.Amount, payMoney, resolvedPayMethodType, resolvedPayMethodName))
...
func WaffoWebhook(c contract.Context) {
	if !isWaffoWebhookEnabled() {
		logger.LogWarn(c.Context(), fmt.Sprintf("Waffo webhook 被拒绝 reason=webhook_disabled path=%q client_ip=%s", c.RequestURI(), c.ClientIP()))
	...
	if err != nil {
		logger.LogError(c.Context(), fmt.Sprintf("Waffo webhook 读取请求体失败 path=%q client_ip=%s error=%q", c.RequestURI(), c.ClientIP(), err.Error()))
	...
	sdk, err := getWaffoSDK()
	if err != nil {
		logger.LogError(c.Context(), fmt.Sprintf("Waffo webhook SDK 初始化失败 path=%q client_ip=%s error=%q", c.RequestURI(), c.ClientIP(), err.Error()))
	...
	logger.LogInfo(c.Context(), fmt.Sprintf("Waffo webhook 收到请求 path=%q client_ip=%s signature=%q body=%q", c.RequestURI(), c.ClientIP(), signature, bodyStr))

	// 验证请求签名
	if !wh.VerifySignature(bodyStr, signature) {
		logger.LogWarn(c.Context(), fmt.Sprintf("Waffo webhook 验签失败 path=%q client_ip=%s signature=%q body=%q", c.RequestURI(), c.ClientIP(), signature, bodyStr))
		c.AbortWithStatus(http.StatusBadRequest)
		return
	}
	...
	if err := common.Unmarshal(bodyBytes, &event); err != nil {
		logger.LogError(c.Context(), fmt.Sprintf("Waffo webhook 解析失败 path=%q client_ip=%s error=%q body=%q", c.RequestURI(), c.ClientIP(), err.Error(), bodyStr))
	...
	case core.EventPayment:
		...
		if err := common.Unmarshal(bodyBytes, &payload); err != nil {
			logger.LogError(c.Context(), fmt.Sprintf("Waffo 支付回调载荷解析失败 event_type=%s client_ip=%s error=%q body=%q", event.EventType, c.ClientIP(), err.Error(), bodyStr))
			sendWaffoWebhookResponse(c, wh, false, "invalid payment payload")
			return
		}
		logger.LogInfo(c.Context(), fmt.Sprintf("Waffo webhook 验签并解析成功 event_type=%s merchant_order_id=%s order_status=%s client_ip=%s", event.EventType, payload.Result.MerchantOrderID, payload.Result.OrderStatus, c.ClientIP()))
		handleWaffoPayment(c, wh, &payload.Result.PaymentNotificationResult)
	default:
		logger.LogInfo(c.Context(), fmt.Sprintf("Waffo webhook 忽略事件 event_type=%s client_ip=%s", event.EventType, c.ClientIP()))
		sendWaffoWebhookResponse(c, wh, true, "")
	}
}

// handleWaffoPayment 处理支付完成通知
func handleWaffoPayment(c contract.Context, wh *core.WebhookHandler, result *core.PaymentNotificationResult) {
	...
	logger.LogInfo(c.Context(), fmt.Sprintf("Waffo 充值成功 trade_no=%s client_ip=%s", merchantOrderId, c.ClientIP()))
	sendWaffoWebhookResponse(c, wh, true, "")
}

// sendWaffoWebhookResponse 发送签名响应
func sendWaffoWebhookResponse(c contract.Context, wh *core.WebhookHandler, success bool, msg string) {
	var body, sig string
	if success {
	...
	c.SetHeader("X-SIGNATURE", sig)
	_ = c.Data(http.StatusOK, "application/json", []byte(body))
}
