...
func TestSubscriptionGroupTransitionsPreserveAuthVersionAndSessions(t *testing.T) {
	...
	require.NoError(t, identity.CreateUserSession(&identity.UserSession{
		SID:             "subscription-auth-session",
		UserID:          user.Id,
		Version:         1,
		UserAuthVersion: 1,
		Status:          identity.UserSessionStatusActive,
	...
	require.NoError(t, identity.RefreshUserGroupCache(user.Id))
	...
	assert.Equal(t, identity.UserSessionStatusActive, session.Status)
	cached, err := identity.GetUserCache(user.Id)
	require.NoError(t, err)
...
func TestSubscriptionGroupCacheRefreshFailureDoesNotChangeCommittedResult(t *testing.T) {
	...
	require.NoError(t, db.AutoMigrate(&identity.User{}, &SubscriptionPlan{}, &UserSubscription{}))
	sqlDB, err := db.DB()
	require.NoError(t, err)
