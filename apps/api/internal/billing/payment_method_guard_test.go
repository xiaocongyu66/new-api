...
func insertUserForPaymentGuardTest(t *testing.T, id int, quota int) *identity.User {
	t.Helper()
	user := &identity.User{
		Id:       id,
...
// truncate installs a fresh per-test database and clears the tables the tiered
// settlement tests write to.
...
func setupIdentityTestDB(t *testing.T) {
	...
	if err := db.AutoMigrate(
		...
		// The billing records these tests charge against. They used to ride on
		// the model package's shared TestMain; billing owns them now.
		&TopUp{},
		&SubscriptionPlan{},
		&SubscriptionOrder{},
		&UserSubscription{},
