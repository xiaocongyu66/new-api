...
var (
	syncDialer     *net.Dialer
	syncTransport  *http.Transport
	syncHTTPClient *http.Client
	syncHTTPOnce   sync.Once
)

func initSyncHTTPClient() {
	syncHTTPOnce.Do(func() {
		syncDialer = &net.Dialer{Timeout: 10 * time.Second}
		syncTransport = &http.Transport{
			MaxIdleConns:          100,
		...
		if common.TLSInsecureSkipVerify {
			syncTransport.TLSClientConfig = common.InsecureTLSConfig
		}
		syncTransport.DialContext = func(ctx context.Context, network, addr string) (net.Conn, error) {
			host, _, err := net.SplitHostPort(addr)
			if err != nil {
				host = addr
			}
			if strings.HasSuffix(host, "github.io") {
				if conn, err := syncDialer.DialContext(ctx, "tcp4", addr); err == nil {
					return conn, nil
				}
				return syncDialer.DialContext(ctx, "tcp6", addr)
			}
			return syncDialer.DialContext(ctx, network, addr)
		}
		syncHTTPClient = &http.Client{Transport: syncTransport}
	})
}
...
// FetchUpstreamRatios fetches ratio data from upstream channels and compares with local.
