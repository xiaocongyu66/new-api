...
func TestExpiredSystemTaskLockFailsOldRunAndClaimsLegacyPendingRun(t *testing.T) {
	...
	require.NoError(t, dbx.DB.Model(&SystemTaskLock{}).
...
func TestExpireStaleSystemTaskLockFailsOldRunAndAllowsNewRun(t *testing.T) {
	...
	var lockCount int64
	require.NoError(t, dbx.DB.Model(&SystemTaskLock{}).Where("task_id = ?", first.TaskID).Count(&lockCount).Error)
...
func TestFinishSystemTaskRetainsExecutor(t *testing.T) {
	...
	var lockCount int64
	require.NoError(t, dbx.DB.Model(&SystemTaskLock{}).Where("task_id = ?", task.TaskID).Count(&lockCount).Error)
	assert.Equal(t, int64(0), lockCount)
}
...
func setupIdentityTestDB(t *testing.T) {
	...
	if err := db.AutoMigrate(
		...
		// ops' own records: this per-test database replaces the one TestMain
		// installs, so it has to carry the system-task tables too.
		&SystemTask{},
		&SystemTaskLock{},
		&SystemInstance{},
	); err != nil {
