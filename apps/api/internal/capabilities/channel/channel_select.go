...
func CacheGetRandomSatisfiedChannel(param *port.SelectParams) (*model.Channel, string, error) {
	...
	if param.TokenGroup == "auto" {
		...
		if lastGroupIndex, exists := common.GetCtxKey(param.Ctx, constant.ContextKeyAutoGroupIndex); exists {
		...
		for i := startGroupIndex; i < len(autoGroups); i++ {
			...
			channel, _ = GetRandomSatisfiedChannel(autoGroup, param.ModelName, priorityRetry, param.RequestPath, param.ExcludeSet)
			if channel == nil {
				// Current group has no available channel for this model, try next group
				// 当前分组没有该模型的可用渠道，尝试下一个分组
				logger.LogDebug(param.Ctx.Context(), "No available channel in group %s for model %s at priorityRetry %d, trying next group", autoGroup, param.ModelName, priorityRetry)
				// 重置状态以尝试下一个分组
			...
			logger.LogDebug(param.Ctx.Context(), "Auto selected group: %s", autoGroup)
			...
			if crossGroupRetry && priorityRetry >= common.RetryTimes {
				...
				logger.LogDebug(param.Ctx.Context(), "Current group %s retries exhausted (priorityRetry=%d >= RetryTimes=%d), preparing switch to next group for next retry", autoGroup, priorityRetry, common.RetryTimes)
				common.SetCtxKey(param.Ctx, constant.ContextKeyAutoGroupIndex, i+1)
				// Reset retry counter so outer loop can continue for next group
				// 重置重试计数器，以便外层循环可以为下一个分组继续
				param.SetRetry(0)
		...
		channel, err = GetRandomSatisfiedChannel(param.TokenGroup, param.ModelName, param.GetRetry(), param.RequestPath, param.ExcludeSet)
		if err != nil {
