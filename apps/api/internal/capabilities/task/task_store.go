...
func GetTimedOutUnfinishedTasks(cutoffUnix int64, limit int) []*model.Task {
	var tasks []*model.Task
	err := model.DB.Where("progress != ?", "100%").
...
func GetAllUnFinishSyncTasks(limit int) []*model.Task {
	var tasks []*model.Task
	err := model.DB.Where("progress != ?", "100%").Where("status != ?", model.TaskStatusFailure).Where("status != ?", model.TaskStatusSuccess).Limit(limit).Order("id").Find(&tasks).Error
	if err != nil {
		return nil
	}
	return tasks
...
func TaskCountAllUserTask(userId int, queryParams model.SyncTaskQueryParams) int64 {
	var total int64
	query := model.DB.Model(&model.Task{}).Where("user_id = ?", userId)
	if queryParams.TaskID != "" {
		query = query.Where("task_id = ?", queryParams.TaskID)
	}
	if queryParams.Action != "" {
