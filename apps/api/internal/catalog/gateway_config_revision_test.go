...
func resetGatewayRevision(t *testing.T, revision int64) {
	t.Helper()
	require.NoError(t, dbx.DB.Exec("DELETE FROM gateway_config_outboxes").Error)
	require.NoError(t, dbx.DB.Exec("DELETE FROM gateway_config_revisions").Error)
	require.NoError(t, dbx.DB.Create(&GatewayConfigRevision{ID: gatewayConfigRevisionID, RoutingRevision: revision}).Error)
	t.Cleanup(func() {
		dbx.DB.Exec("DELETE FROM gateway_config_outboxes")
		dbx.DB.Exec("DELETE FROM gateway_config_revisions")
	})
}
...
func currentGatewayRevision(t *testing.T) int64 {
	...
	require.NoError(t, dbx.DB.Where("id = ?", gatewayConfigRevisionID).Take(&row).Error)
...
func outboxRevisions(t *testing.T) []int64 {
	t.Helper()
	var revisions []int64
	require.NoError(t, dbx.DB.Model(&GatewayConfigOutbox{}).
...
func TestGatewayConfigRevisionInitializeKeepsExistingWatermark(t *testing.T) {
	...
	var rows int64
	require.NoError(t, dbx.DB.Model(&GatewayConfigRevision{}).Count(&rows).Error)
...
func TestGatewayConfigRevisionInitializeRejectsNonCanonicalRow(t *testing.T) {
	...
	require.NoError(t, dbx.DB.Create(&GatewayConfigRevision{ID: 9, RoutingRevision: 42}).Error)

	err := InitializeGatewayConfigRevision()
	require.ErrorIs(t, err, ErrGatewayRevisionMissing)

	var rows []GatewayConfigRevision
	require.NoError(t, dbx.DB.Order("id asc").Find(&rows).Error)
...
func TestGatewayConfigRevisionCommitsDomainRowRevisionAndOutboxTogether(t *testing.T) {
	...
	require.NoError(t, err)
	t.Cleanup(func() { dbx.DB.Where("name = ?", "revision-probe").Delete(&Channel{}) })
	...
	var channel Channel
	require.NoError(t, dbx.DB.Where("name = ?", "revision-probe").Take(&channel).Error)
	assert.Equal(t, "gateway-revision-commit-probe", channel.Key)

	var outbox GatewayConfigOutbox
	require.NoError(t, dbx.DB.Where("routing_revision = ?", revision).Take(&outbox).Error)
...
func TestGatewayConfigRevisionRollsBackEverythingOnMutatorFailure(t *testing.T) {
	...
	var found int64
	require.NoError(t, dbx.DB.Model(&Channel{}).Where("name = ?", "revision-rollback-probe").Count(&found).Error)
...
func TestGatewayConfigOutboxRejectsDuplicateRevision(t *testing.T) {
	...
	err = dbx.DB.Create(&GatewayConfigOutbox{RoutingRevision: revision}).Error
