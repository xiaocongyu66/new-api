...
func IsModelNameDuplicated(id int, name string) (bool, error) {
	...
	var cnt int64
	err := dbx.DB.Model(&Model{}).Where("model_name = ? AND id <> ?", name, id).Count(&cnt).Error
	return cnt > 0, err
}
...
func GetPreferredModelOwnerChannelTypes(modelNames []string, groups []string) (map[string]int, error) {
	...
	query := dbx.DB.Table("abilities").
		Select("abilities.model as model, channels.type as channel_type").
		Joins("JOIN channels ON abilities.channel_id = channels.id").
		Where("abilities.model IN ? AND abilities.enabled = ? AND channels.status = ?", modelNames, true, common.ChannelStatusEnabled).
		// Ordering used to be priority then weight; both ability columns are gone
		// with the tier scheduler. This query only picks which channel type is
		// displayed as a model's owner, so the lowest channel id is a stable,
		// deterministic choice.
		Order("abilities.channel_id ASC")

	groups = normalizeLookupValues(groups)
	if len(groups) > 0 {
		query = query.Where("abilities."+dbx.GroupCol()+" IN ?", groups)
	}
