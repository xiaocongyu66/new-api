...
func PublishPendingGatewayRevisions(ctx context.Context, limit int) error {
	rows, err := ListPendingGatewayConfigOutbox(limit)
	if err != nil {
		return err
	}
	for _, row := range rows {
		if err := PublishGatewayRoutingRevision(ctx, row.RoutingRevision); err != nil {
			if recordErr := RecordGatewayConfigOutboxAttempt(row.ID, classifyGatewayOutboxError(err)); recordErr != nil {
				return recordErr
			}
			continue
		}
		if err := RecordGatewayConfigOutboxAttempt(row.ID, "published"); err != nil {
			return err
		}
		if err := MarkGatewayConfigOutboxPublished(row.ID, time.Now()); err != nil {
			return err
		}
	}
	return nil
