...
func TestSyncIdempotent(t *testing.T) {
	...
	var count1 int64
	require.NoError(t, dbx.DB.Model(&ChannelModelRoute{}).Where("channel_id = ?", 1).Count(&count1).Error)
	require.Equal(t, int64(2), count1)

	// Second sync
	require.NoError(t, SyncChannelModelRoutesWithTx(dbx.DB, 1))
	var count2 int64
	require.NoError(t, dbx.DB.Model(&ChannelModelRoute{}).Where("channel_id = ?", 1).Count(&count2).Error)
	require.Equal(t, int64(2), count2)

	// Content identical
	var routes1, routes2 []ChannelModelRoute
	require.NoError(t, dbx.DB.Where("channel_id = ?", 1).Find(&routes1).Error)
...
func TestSyncStaleCleanup(t *testing.T) {
	...
	ch.Models = "alias-a"
	require.NoError(t, dbx.DB.Save(ch).Error)
	require.NoError(t, SyncChannelModelRoutesWithTx(dbx.DB, 1))
	...
	require.NoError(t, dbx.DB.Create(ch2).Error)
	require.NoError(t, SyncChannelModelRoutesWithTx(dbx.DB, 2))

	require.NoError(t, dbx.DB.Model(&ChannelModelRoute{}).Where("channel_id = ?", 2).Count(&count).Error)
	require.Equal(t, int64(3), count)

	// Reduce to 1 key
	ch2.Key = "k1-only"
	ch2.ChannelInfo.MultiKeySize = 1
	require.NoError(t, dbx.DB.Save(ch2).Error)
	require.NoError(t, SyncChannelModelRoutesWithTx(dbx.DB, 2))
	...
	require.NoError(t, dbx.DB.Where("channel_id = ?", 2).First(&r2).Error)
	assert.Equal(t, 0, r2.KeyIndex)

	// Delete channel row → routes cleared
	require.NoError(t, dbx.DB.Delete(&Channel{}, 1).Error)
	require.NoError(t, SyncChannelModelRoutesWithTx(dbx.DB, 1))
...
func TestUniqueConstraintNoDuplicate(t *testing.T) {
	...
	require.NoError(t, dbx.DB.Clauses(clause.OnConflict{DoNothing: true}).Create(&dup).Error)
	...
	require.NoError(t, dbx.DB.Where("channel_id = ?", 1).First(&r).Error)
...
func TestChannelLifecycleSyncsRouteRows(t *testing.T) {
	...
	ch := makeSingleKeyChannel(1, "model-a,model-b", "group-1", nil)
	require.NoError(t, ch.insertWithTx(dbx.DB))
	...
	ch.Models = "model-a"
	require.NoError(t, ch.updateWithTx(dbx.DB))
...
func TestGetRouteUnitViewsByAlias(t *testing.T) {
	...
	require.NoError(t, dbx.DB.Create(ch3).Error)
	require.NoError(t, SeedChannelModelRoutes())
...
func TestUpdateRouteUnitConfig(t *testing.T) {
	...
	require.NoError(t, dbx.DB.Where("channel_id = ?", 1).First(&route).Error)
	routeID := route.Id
	...
	require.NoError(t, dbx.DB.First(&updated, routeID).Error)
