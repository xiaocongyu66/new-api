...
var storeChannelPollingLocks sync.Map
...
func channelPollingLock(channelId int) *sync.Mutex {
	...
	if lock, exists := storeChannelPollingLocks.Load(channelId); exists {
		return lock.(*sync.Mutex)
	}
	newLock := &sync.Mutex{}
	actual, _ := storeChannelPollingLocks.LoadOrStore(channelId, newLock)
...
// StoreUpdateChannelStatus implementation. Called once by
...
// StoreUpdateChannelStatus mutates the channel status (and, for multi-key channels,
...
func StoreUpdateChannelStatus(channelId int, usingKey string, status int, reason string) bool {
...
// storeChannelStatusLock guards the cache-refresh section of the legacy
// StoreUpdateChannelStatus fallback. The capability owns its own copy.
var storeChannelStatusLock sync.Mutex
...
func legacyUpdateChannelStatus(channelId int, usingKey string, status int, reason string) bool {
	if common.MemoryCacheEnabled {
		storeChannelStatusLock.Lock()
		defer storeChannelStatusLock.Unlock()
	}
