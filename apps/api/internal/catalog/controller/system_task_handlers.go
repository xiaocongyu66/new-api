...
// opsservice.StartSystemTaskRunner.
func RegisterScheduledSystemTasks() {
	rootservice.RegisterSystemTaskHandler(channelTestHandler{})
	rootservice.RegisterSystemTaskHandler(modelUpdateHandler{})
	rootservice.RegisterSystemTaskHandler(midjourneyPollHandler{})
	rootservice.RegisterSystemTaskHandler(asyncTaskPollHandler{})
}
...
func (channelTestHandler) Type() string { return opsmodel.SystemTaskTypeChannelTest }
...
func (channelTestHandler) Run(ctx context.Context, task *opsmodel.SystemTask, runnerID string) {
	payload := ChannelTestTaskPayload{}
	if err := task.DecodePayload(&payload); err != nil {
		finishSystemTaskHandler(task, runnerID, opsmodel.SystemTaskStatusFailed, nil, err)
		return
	}
	summary, err := RunChannelTestTask(ctx, payload.Mode, payload.Notify, rootservice.NewSystemTaskProgressReporter(task, runnerID))
	if err != nil {
		finishSystemTaskHandler(task, runnerID, opsmodel.SystemTaskStatusFailed, nil, err)
		return
	}
	finishSystemTaskHandler(task, runnerID, opsmodel.SystemTaskStatusSucceeded, summary, nil)
}

// modelUpdateHandler runs the scheduled upstream model update detection job.
type modelUpdateHandler struct{}

func (modelUpdateHandler) Type() string { return opsmodel.SystemTaskTypeModelUpdate }
...
func (modelUpdateHandler) Run(ctx context.Context, task *opsmodel.SystemTask, runnerID string) {
	payload := modelUpdateTaskPayload{}
	if err := task.DecodePayload(&payload); err != nil {
		finishSystemTaskHandler(task, runnerID, opsmodel.SystemTaskStatusFailed, nil, err)
		return
	}
	summary := RunChannelUpstreamModelUpdateTaskOnce(ctx, payload.Manual, !payload.Manual, rootservice.NewSystemTaskProgressReporter(task, runnerID))
	finishSystemTaskHandler(task, runnerID, opsmodel.SystemTaskStatusSucceeded, summary, nil)
}
...
func (midjourneyPollHandler) Type() string { return opsmodel.SystemTaskTypeMidjourneyPoll }

func (midjourneyPollHandler) Enabled() bool {
	return constant.UpdateTask && taskmodel.HasUnfinishedMidjourneyTasks()
}
...
func (midjourneyPollHandler) Run(ctx context.Context, task *opsmodel.SystemTask, runnerID string) {
	summary := taskservice.RunMidjourneyTaskUpdateOnce(ctx, rootservice.NewSystemTaskProgressReporter(task, runnerID))
	finishSystemTaskHandler(task, runnerID, opsmodel.SystemTaskStatusSucceeded, summary, nil)
}
...
func (asyncTaskPollHandler) Type() string { return opsmodel.SystemTaskTypeAsyncTaskPoll }

func (asyncTaskPollHandler) Enabled() bool {
	return constant.UpdateTask && taskmodel.HasUnfinishedSyncTasks()
}
...
func (asyncTaskPollHandler) Run(ctx context.Context, task *opsmodel.SystemTask, runnerID string) {
	summary := taskservice.RunTaskPollingOnce(ctx, rootservice.NewSystemTaskProgressReporter(task, runnerID))
	finishSystemTaskHandler(task, runnerID, opsmodel.SystemTaskStatusSucceeded, summary, nil)
}

func finishSystemTaskHandler(task *opsmodel.SystemTask, runnerID string, status opsmodel.SystemTaskStatus, result any, runErr error) {
	errorMessage := ""
	if runErr != nil {
		errorMessage = runErr.Error()
	}
	if err := opsmodel.FinishSystemTask(task.TaskID, runnerID, status, result, errorMessage); err != nil {
