...
func CreateUserSession(session *UserSession) error {
	...
	if err := dbx.DB.Create(session).Error; err != nil {
		return err
	}
...
func CountUserSessionsCreatedSince(userID int, createdAfter int64) (int64, error) {
	...
	query := dbx.DB.Model(&UserSession{}).Where("created_at > ?", createdAfter)
	if userID > 0 {
...
func GetUserSessionBySID(sid string) (*UserSession, error) {
	...
	if err := dbx.DB.Where("sid = ?", sid).First(&session).Error; err != nil {
		return nil, err
	}
	return &session, nil
}
...
func ListActiveUserSessions(userID int, currentSID string, now int64) ([]UserSession, error) {
	...
	if err := dbx.DB.Model(&User{}).Where("id = ?", userID).Select("auth_version").Find(&authVersion).Error; err != nil {
		return nil, err
	}
	if authVersion <= 0 {
		return nil, ErrUserSessionInvalid
...
// is a compare-and-swap so SQLite, where dbx.LockForUpdate is intentionally a
...
func RotateUserSessionRefresh(userID int, sid, presentedHash, nextHash string, now int64, grace time.Duration) (*UserSession, error) {
	...
	for range 3 {
		cacheDeadline := userSessionCacheDeadline()
		var session UserSession
		if err := dbx.DB.Where("sid = ? AND user_id = ?", sid, userID).First(&session).Error; err != nil {
			return nil, err
		}
		if session.Status != UserSessionStatusActive || session.RevokedAt != 0 || session.ExpiresAt <= now {
			return nil, ErrUserSessionInactive
		...
		if hmac.Equal([]byte(session.RefreshHash), []byte(presentedHash)) {
			result := dbx.DB.Model(&UserSession{}).
...
func RevokeUserSession(userID int, sid, reason string) (bool, error) {
	...
	if err := dbx.DB.Where("sid = ? AND user_id = ?", sid, userID).First(&candidate).Error; err != nil {
	...
	err := dbx.DB.Transaction(func(tx *gorm.DB) error {
		var current UserSession
		if err := dbx.LockForUpdate(tx).Where("sid = ? AND user_id = ?", sid, userID).First(&current).Error; err != nil {
			return err
		}
...
func RevokeUserSessionByRefreshHash(sid, presentedHash, reason string) (bool, error) {
	...
	var revoked bool
	err := dbx.DB.Transaction(func(tx *gorm.DB) error {
		if err := dbx.LockForUpdate(tx).Where("sid = ?", sid).First(&session).Error; err != nil {
			if errors.Is(err, gorm.ErrRecordNotFound) {
				return nil
			}
			return err
...
func AdvanceUserSessionAuthVersion(userID int, sid string, expectedSessionVersion, expectedUserAuthVersion, nextUserAuthVersion int64) (*UserSession, error) {
	...
	var session UserSession
	err := dbx.DB.Transaction(func(tx *gorm.DB) error {
		if err := dbx.LockForUpdate(tx).Where("sid = ? AND user_id = ?", sid, userID).First(&session).Error; err != nil {
			return err
		}
...
func revokeUserSessions(userID int, excludedSID, reason string) (int64, error) {
	...
	for {
		query := dbx.DB.Where("user_id = ? AND status = ? AND expires_at > ?", userID, UserSessionStatusActive, now)
		if excludedSID != "" {
		...
		err := dbx.DB.Transaction(func(tx *gorm.DB) error {
			if err := dbx.LockForUpdate(tx).Where("sid IN ? AND status = ?", sids, UserSessionStatusActive).Find(&revoked).Error; err != nil {
				return err
			}
			if len(revoked) == 0 {
				return nil
...
func deleteExpiredUserSessionsBefore(expiredBefore, issuanceCutoff, revokedBefore int64) error {
	for {
		var sids []string
		if err := dbx.DB.Model(&UserSession{}).
			Where(
		...
		for start := 0; start < len(sids); start += userSessionCleanupBatchSize {
			...
			if err := dbx.DB.Where("sid IN ?", sids[start:end]).
				Where(
					"expires_at < ? AND created_at <= ? AND (status <> ? OR revoked_at <= 0 OR revoked_at < ?)",
					expiredBefore,
					issuanceCutoff,
