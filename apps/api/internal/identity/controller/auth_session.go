...
func RefreshAuth(c *gin.Context) {
	setAuthNoStore(c)
	rawRefreshToken, err := c.Cookie(identityservice.RefreshCookieName)
	if err != nil || rawRefreshToken == "" {
		identityservice.ClearRefreshCookie(c)
		writeAuthSessionError(c, identityservice.ErrRefreshTokenInvalid)
		return
	}
	bundle, user, err := identityservice.RefreshLoginSession(rawRefreshToken, c.GetHeader("X-Auth-Session"), c.ClientIP(), c.Request.UserAgent())
	if err != nil {
		if errors.Is(err, identityservice.ErrRefreshTokenInvalid) || errors.Is(err, identityservice.ErrLoginSessionRevoked) {
			identityservice.ClearRefreshCookie(c)
		}
...
func AuthLogout(c *gin.Context) {
	...
	rawRefreshToken, cookieErr := c.Cookie(identityservice.RefreshCookieName)
	cookieSID, hasCookieSID := identityservice.RefreshTokenSID(rawRefreshToken)
	if expectedSID != "" && cookieErr == nil && hasCookieSID && cookieSID != expectedSID {
		writeAuthSessionError(c, identityservice.ErrLoginSessionMismatch)
		return
	}

	if rawAccessToken, ok := dashboardBearer(c.GetHeader("Authorization")); ok {
		if identity, err := identityservice.ParseAccessToken(rawAccessToken); err == nil {
			if expectedSID != "" && expectedSID != identity.SessionID {
				writeAuthSessionError(c, identityservice.ErrLoginSessionMismatch)
				return
			}
			if _, err := identitymodel.RevokeUserSession(identity.UserID, identity.SessionID, "logout"); err != nil {
				writeAuthSessionError(c, err)
				return
			}
			cookieCleared := false
			if cookieErr == nil && hasCookieSID && cookieSID == identity.SessionID {
				if err := identityservice.RevokeByRefreshToken(rawRefreshToken, identity.SessionID, "logout"); err != nil {
					writeAuthSessionError(c, err)
					return
				}
				identityservice.ClearRefreshCookie(c)
	...
	if err := identityservice.RevokeByRefreshToken(rawRefreshToken, expectedSID, "logout"); err != nil {
		writeAuthSessionError(c, err)
		return
	}
	identityservice.ClearRefreshCookie(c)
...
func GetLoginSessions(c *gin.Context) {
	...
	sessions, err := identityservice.ListLoginSessions(identity.UserID, identity.SessionID)
	if err != nil {
		writeAuthSessionError(c, err)
		return
	}
...
func DeleteLoginSession(c *gin.Context) {
	...
	revoked, err := identitymodel.RevokeUserSession(identity.UserID, sid, "user_revoked")
	if err != nil {
		writeAuthSessionError(c, err)
		return
	}
	...
	if rawRefreshToken, cookieErr := c.Cookie(identityservice.RefreshCookieName); cookieErr == nil {
		cookieSID, ok := identityservice.RefreshTokenSID(rawRefreshToken)
		if ok && cookieSID == sid {
			identityservice.ClearRefreshCookie(c)
		}
	}
...
func RevokeOtherLoginSessions(c *gin.Context) {
	...
	count, err := identitymodel.RevokeOtherUserSessions(identity.UserID, identity.SessionID, "user_revoked_others")
	if err != nil {
		writeAuthSessionError(c, err)
		return
	}
...
func requireBrowserSession(c *gin.Context) (identityservice.AuthIdentity, bool) {
...
func writeAuthSessionError(c *gin.Context, err error) {
	status, code := identityservice.AuthSessionErrorCode(err)
...
func authRotationData(bundle *identityservice.AuthBundle) gin.H {
	return gin.H{
		"access_token":      bundle.AccessToken,
		"token_type":        bundle.TokenType,
		"access_expires_at": bundle.AccessExpiresAt,
