...
func TestExternalIdentityClaimEnforcesSingleOwnerAtomically(t *testing.T) {
	...
	require.NoError(t, rootmodel.DB.Create(&first).Error)
	require.NoError(t, rootmodel.DB.Create(&second).Error)
	...
	require.NoError(t, rootmodel.DB.Find(&claims).Error)
...
func TestClearTelegramBindingReleasesIdentityClaim(t *testing.T) {
	...
	var count int64
	require.NoError(t, rootmodel.DB.Model(&ExternalIdentityClaim{}).Where("user_id = ?", user.Id).Count(&count).Error)
	assert.Zero(t, count)
}
...
func TestInitializeExternalIdentityClaimsIsIdempotent(t *testing.T) {
	...
	require.NoError(t, rootmodel.DB.Where("provider = ? AND subject = ?", ExternalIdentityProviderTelegram, user.TelegramId).
		First(&claim).Error)
...
func TestInitializeExternalIdentityClaimsRejectsAmbiguousLegacyBindings(t *testing.T) {
	...
	var count int64
	require.NoError(t, rootmodel.DB.Model(&ExternalIdentityClaim{}).Count(&count).Error)
	assert.Zero(t, count)
}
