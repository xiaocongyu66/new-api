// Package httperr renders the request-rejection envelopes shared by every layer
...
// two response shapes. A change to an envelope had to be remembered in each
// copy, and a missed one meant clients saw a different error format depending
// on which layer rejected the call. The package sits above internal/logger on
// purpose: internal/common cannot host these helpers because logger already
...
// AbortWithMidjourneyMessage rejects a request with the Midjourney proxy
// envelope, which carries `description` plus a numeric `code` instead of the
// OpenAI `error` object.
