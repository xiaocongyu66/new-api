...
func GetChannelAffinityCacheStats(c contract.Context) {
	stats := channelpkg.GetChannelAffinityCacheStats()
...
func ClearChannelAffinityCache(c contract.Context) {
	...
	if all == "true" {
		deleted := channel.ClearChannelAffinityCacheAll()
		_ = c.JSON(http.StatusOK, common.H{
			"success": true,
			"message": "",
			"data": common.H{
	...
	deleted, err := channel.ClearChannelAffinityCacheByRuleName(ruleName)
	if err != nil {
		_ = c.JSON(http.StatusBadRequest, common.H{
			"success": false,
			"message": err.Error(),
...
func GetChannelAffinityUsageCacheStats(c contract.Context) {
	...
	stats := channelpkg.GetChannelAffinityUsageCacheStats(ruleName, usingGroup, keyFp)
	_ = c.JSON(http.StatusOK, common.H{
		"success": true,
		"message": "",
		"data":    stats,
