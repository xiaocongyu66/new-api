...
func clearChannelInfo(channel *channelpkg.Channel) {
...
func buildChannelListQuery(group string, statusFilter int, typeFilter int) *gorm.DB {
	query := dbx.DB.Model(&channelpkg.Channel{})
	query = channelpkg.ApplyChannelGroupFilter(query, group)
...
func GetChannelOps(c contract.Context) {
	common.CtxApiSuccess(c, common.H{
		"retry_times": common.RetryTimes,
	})
}

func GetAllChannels(c contract.Context) {
	pageInfo := common.GetPageQuery(c)
	channelData := make([]*channelpkg.Channel, 0)
	idSort, _ := strconv.ParseBool(c.Query("id_sort"))
	sortOptions := channelpkg.NewChannelSortOptions(c.Query("sort_by"), c.Query("sort_order"), idSort)
	enableTagMode, _ := strconv.ParseBool(c.Query("tag_mode"))
	groupFilter := channelpkg.NormalizeChannelGroupFilter(c.Query("group"))
	...
	if enableTagMode {
		tags, err := channelpkg.GetPaginatedChannelTags(buildChannelListQuery(groupFilter, statusFilter, typeFilter), pageInfo.GetStartIdx(), pageInfo.GetPageSize())
		if err != nil {
			common.SysError("failed to get paginated tags: " + err.Error())
			_ = c.JSON(http.StatusOK, common.H{"success": false, "message": "获取标签失败，请稍后重试"})
			return
		}
		total, err = channelpkg.CountChannelTags(buildChannelListQuery(groupFilter, statusFilter, typeFilter))
		if err != nil {
			common.SysError("failed to count tags: " + err.Error())
			_ = c.JSON(http.StatusOK, common.H{"success": false, "message": "获取标签数量失败，请稍后重试"})
			return
		}
		for _, tag := range tags {
			...
			var tagChannels []*channelpkg.Channel
			...
			if err != nil {
				...
				_ = c.JSON(http.StatusOK, common.H{"success": false, "message": "获取标签渠道失败，请稍后重试"})
				return
			}
		...
		if err := buildChannelListQuery(groupFilter, statusFilter, typeFilter).Count(&total).Error; err != nil {
			...
			_ = c.JSON(http.StatusOK, common.H{"success": false, "message": "获取渠道数量失败，请稍后重试"})
			return
		}
		...
		if err != nil {
			...
			_ = c.JSON(http.StatusOK, common.H{"success": false, "message": "获取渠道列表失败，请稍后重试"})
			return
		}
	}
	...
	if err := countQuery.Select("type, count(*) as count").Group("type").Find(&results).Error; err != nil {
		...
		_ = c.JSON(http.StatusOK, common.H{"success": false, "message": "获取渠道类型统计失败，请稍后重试"})
		return
	}
...
func buildFetchModelsHeaders(channel *channelpkg.Channel, key string) (http.Header, error) {
...
func applyFetchModelsHeaderOverrides(channel *channelpkg.Channel, key string, headers http.Header) error {
...
func FetchUpstreamModels(c contract.Context) {
	...
	channel, err := channelpkg.GetChannelById(id, true)
	if err != nil {
		common.CtxApiError(c, err)
		return
	}
...
func FixChannelsAbilities(c contract.Context) {
	success, fails, err := channelpkg.FixAbility()
	if err != nil {
		common.CtxApiError(c, err)
		return
	}
...
func SearchChannels(c contract.Context) {
	...
	if enableTagMode {
		...
		channels, err := channelpkg.SearchChannels(keyword, group, modelKeyword, idSort, sortOptions)
		if err != nil {
			_ = c.JSON(http.StatusOK, common.H{
				"success": false,
				"message": err.Error(),
	...
	if statusFilter == common.ChannelStatusEnabled || statusFilter == 0 {
		filtered := make([]*channelpkg.Channel, 0, len(channelData))
...
func GetChannel(c contract.Context) {
	id, err := strconv.Atoi(c.Param("id"))
	if err != nil {
		common.CtxApiError(c, err)
		return
	}
	channel, err := channelpkg.GetChannelById(id, false)
	if err != nil {
		common.CtxApiError(c, err)
		return
	}
...
func GetChannelKey(c contract.Context) {
	channelId, err := strconv.Atoi(c.Param("id"))
	if err != nil {
		common.CtxApiError(c, fmt.Errorf("渠道ID格式错误: %v", err))
		return
	}

	// 获取渠道信息（包含密钥）
	channel, err := channelpkg.GetChannelById(channelId, true)
	if err != nil {
		common.CtxApiError(c, fmt.Errorf("获取渠道信息失败: %v", err))
		return
	}

	if channel == nil {
		common.CtxApiError(c, fmt.Errorf("渠道不存在"))
		return
	}

	// 记录操作审计日志（高危：查看渠道密钥）
	usage.RecordManageAudit(c, "channel.key_view", map[string]interface{}{
		"id":   channelId,
		"name": channel.Name,
	})
...
func validateChannel(channel *channelpkg.Channel, isAdd bool) error {
	if channel == nil {
...
func RefreshCodexChannelCredential(c contract.Context) {
	channelId, err := strconv.Atoi(c.Param("id"))
	if err != nil {
	...
	oauthKey, ch, err := channelpkg.RefreshCodexChannelCredential(ctx, channelId, channelpkg.CodexCredentialRefreshOptions{ResetCaches: true})
	if err != nil {
		common.SysError("failed to refresh codex channel credential: " + err.Error())
		_ = c.JSON(http.StatusOK, common.H{"success": false, "message": "刷新凭证失败，请稍后重试"})
		return
	}
...
type AddChannelRequest struct {
	...
	Channel                   *channelpkg.Channel   `json:"channel"`
}
...
func AddChannel(c contract.Context) {
	addChannelRequest := AddChannelRequest{}
	err := c.BindJSON(&addChannelRequest)
	if err != nil {
		common.CtxApiError(c, err)
		return
	}
	...
	channels := make([]channelpkg.Channel, 0, len(keys))
	...
	err = channelpkg.BatchInsertChannels(channels)
	if err != nil {
		common.CtxApiError(c, err)
		return
	}
	usage.RecordManageAudit(c, "channel.create", map[string]interface{}{
...
func DeleteChannel(c contract.Context) {
	id, _ := strconv.Atoi(c.Param("id"))
	channelName := ""
	channelProxy := ""
	channelLookupFailed := false
	if existing, err := channelpkg.GetChannelById(id, false); err == nil && existing != nil {
		channelName = existing.Name
		channelProxy = existing.GetSetting().Proxy
	} else {
		channelLookupFailed = true
	}
	channel := channelpkg.Channel{Id: id}
	err := channel.Delete()
	if err != nil {
		common.CtxApiError(c, err)
		return
...
func DeleteDisabledChannel(c contract.Context) {
	rows, err := channelpkg.DeleteDisabledChannel()
	if err != nil {
		common.CtxApiError(c, err)
		return
	}
...
func DisableTagChannels(c contract.Context) {
	channelTag := ChannelTag{}
	err := c.BindJSON(&channelTag)
	...
	err = channelpkg.DisableChannelByTag(channelTag.Tag)
	if err != nil {
		common.CtxApiError(c, err)
		return
	}
	channelpkg.InitChannelCache()
	usage.RecordManageAudit(c, "channel.tag_disable", map[string]interface{}{
		"tag": channelTag.Tag,
	})
	_ = c.JSON(http.StatusOK, common.H{
		"success": true,
...
func EnableTagChannels(c contract.Context) {
	channelTag := ChannelTag{}
	err := c.BindJSON(&channelTag)
	if err != nil || channelTag.Tag == "" {
		_ = c.JSON(http.StatusOK, common.H{
	...
	err = channelpkg.EnableChannelByTag(channelTag.Tag)
	if err != nil {
		common.CtxApiError(c, err)
		return
	}
	channelpkg.InitChannelCache()
	usage.RecordManageAudit(c, "channel.tag_enable", map[string]interface{}{
		"tag": channelTag.Tag,
	})
	_ = c.JSON(http.StatusOK, common.H{
		"success": true,
...
func EditTagChannels(c contract.Context) {
	channelTag := ChannelTag{}
	err := c.BindJSON(&channelTag)
	if err != nil {
		_ = c.JSON(http.StatusOK, common.H{
	...
	err = channelpkg.EditChannelByTag(channelTag.Tag, channelTag.NewTag, channelTag.ModelMapping, channelTag.Models, channelTag.Groups, channelTag.Priority, channelTag.Weight, channelTag.ParamOverride, channelTag.HeaderOverride)
	if err != nil {
		common.CtxApiError(c, err)
		return
	}
	channelpkg.InitChannelCache()
	usage.RecordManageAudit(c, "channel.tag_edit", map[string]interface{}{
		"tag": channelTag.Tag,
	})
	_ = c.JSON(http.StatusOK, common.H{
		"success": true,
...
func DeleteChannelBatch(c contract.Context) {
	channelBatch := ChannelBatch{}
	err := c.BindJSON(&channelBatch)
	...
	deletedCount, err := channelpkg.BatchDeleteChannels(channelBatch.Ids)
	if err != nil {
		common.CtxApiError(c, err)
		return
	}
...
func UpdateChannel(c contract.Context) {
	...
	// Preserve existing channelpkg.ChannelInfo to ensure multi-key channels keep correct state even if the client does not send channelpkg.ChannelInfo in the request.
	originChannel, err := channelpkg.GetChannelById(channel.Id, true)
	if err != nil {
		_ = c.JSON(http.StatusOK, common.H{
			"success": false,
			"message": err.Error(),
	...
	// Always copy the original channelpkg.ChannelInfo so that fields like IsMultiKey and MultiKeySize are retained.
	...
	usage.RecordManageAudit(c, "channel.update", map[string]interface{}{
		"id":             channel.Id,
		"name":           channel.Name,
...
func UpdateChannelStatus(c contract.Context) {
	id, err := strconv.Atoi(c.Param("id"))
	if err != nil {
		common.CtxApiErrorI18n(c, i18n.MsgInvalidParams)
		return
	...
	if err := c.BindJSON(&req); err != nil || !isManageableChannelStatus(req.Status) {
		common.CtxApiErrorI18n(c, i18n.MsgInvalidParams)
		return
	}
	changed := channelpkg.UpdateChannelStatus(id, "", req.Status, "manual operation")
	if changed {
		channelpkg.InitChannelCache()
	}
	usage.RecordManageAudit(c, "channel.status_update", map[string]interface{}{
		"id":      id,
		"status":  req.Status,
		"changed": changed,
	})
...
func BatchUpdateChannelStatus(c contract.Context) {
	req := ChannelStatusBatchRequest{}
	if err := c.BindJSON(&req); err != nil || len(req.Ids) == 0 || !isManageableChannelStatus(req.Status) {
		common.CtxApiErrorI18n(c, i18n.MsgInvalidParams)
		return
	}
	changedCount := 0
	for _, id := range req.Ids {
		if channelpkg.UpdateChannelStatus(id, "", req.Status, "manual batch operation") {
			changedCount++
		}
	}
	if changedCount > 0 {
	...
	usage.RecordManageAudit(c, "channel.status_update_batch", map[string]interface{}{
		"count":  changedCount,
		"total":  len(req.Ids),
		"status": req.Status,
	})
...
func buildAdvancedCustomModelPreviewChannel(req fetchModelsRequest) (*channelpkg.Channel, error) {
	var channel *channelpkg.Channel
	if req.ChannelID > 0 {
		savedChannel, err := channelpkg.GetChannelById(req.ChannelID, true)
		if err != nil {
			return nil, err
		}
...
func FetchModels(c contract.Context) {
...
func BatchSetChannelTag(c contract.Context) {
	channelBatch := ChannelBatch{}
	err := c.BindJSON(&channelBatch)
	if err != nil || len(channelBatch.Ids) == 0 {
		_ = c.JSON(http.StatusOK, common.H{
	...
	err = channelpkg.BatchSetChannelTag(channelBatch.Ids, channelBatch.Tag)
	if err != nil {
		common.CtxApiError(c, err)
		return
	}
	channelpkg.InitChannelCache()
	usage.RecordManageAudit(c, "channel.tag_batch_set", map[string]interface{}{
...
func GetTagModels(c contract.Context) {
	tag := c.Query("tag")
	if tag == "" {
		_ = c.JSON(http.StatusBadRequest, common.H{
			"success": false,
	...
	channels, err := channelpkg.GetChannelsByTag(tag, false, false) // idSort=false, selectAll=false
	if err != nil {
...
func CopyChannel(c contract.Context) {
	id, err := strconv.Atoi(c.Param("id"))
	if err != nil {
		_ = c.JSON(http.StatusOK, common.H{"success": false, "message": "invalid id"})
		return
	}
	...
	origin, err := channelpkg.GetChannelById(id, true)
	if err != nil {
		common.SysError("failed to get channel by id: " + err.Error())
		_ = c.JSON(http.StatusOK, common.H{"success": false, "message": "获取渠道信息失败，请稍后重试"})
		return
	}
	...
	if err := clone.ValidateSettings(); err != nil {
		...
		_ = c.JSON(http.StatusOK, common.H{"success": false, "message": "Failed to copy channel: invalid channel settings"})
		return
	}
	...
	if err := clone.Insert(); err != nil {
		...
		_ = c.JSON(http.StatusOK, common.H{"success": false, "message": "复制渠道失败，请稍后重试"})
		return
	}
	channelpkg.InitChannelCache()
	usage.RecordManageAudit(c, "channel.copy", map[string]interface{}{
		"sourceId": id,
		"id":       clone.Id,
		"name":     clone.Name,
	})
	// success
	_ = c.JSON(http.StatusOK, common.H{"success": true, "message": "", "data": common.H{"id": clone.Id}})
}
...
func ManageMultiKeys(c contract.Context) {
	...
	channel, err := channelpkg.GetChannelById(request.ChannelId, true)
	if err != nil {
		_ = c.JSON(http.StatusOK, common.H{
			"success": false,
			"message": "渠道不存在",
	...
	if request.Action == "get_key_status" {
		usage.MarkAuditLogged(c)
	} else {
		usage.RecordManageAudit(c, "channel.multi_key_manage", map[string]interface{}{
			"action": request.Action,
			"id":     channel.Id,
		})
	}

	lock := channelpkg.GetChannelPollingLock(channel.Id)
	lock.Lock()
	defer lock.Unlock()
...
func OllamaPullModel(c contract.Context) {
	var req struct {
		ChannelID int    `json:"channel_id"`
	...
	if req.ChannelID == 0 || req.ModelName == "" {
		_ = c.JSON(http.StatusBadRequest, common.H{
			"success": false,
			"message": "channelpkg.Channel ID and model name are required",
		})
		return
	}

	// 获取渠道信息
	channel, err := channelpkg.GetChannelById(req.ChannelID, true)
	if err != nil {
		_ = c.JSON(http.StatusNotFound, common.H{
			"success": false,
			"message": "channelpkg.Channel not found",
		})
		return
	}
...
func OllamaPullModelStream(c contract.Context) {
	var req struct {
		ChannelID int    `json:"channel_id"`
		ModelName string `json:"model_name"`
	}
...
func OllamaDeleteModel(c contract.Context) {
	var req struct {
		ChannelID int    `json:"channel_id"`
		ModelName string `json:"model_name"`
	}
	...
	_ = c.JSON(http.StatusOK, common.H{
		"success": true,
		"message": fmt.Sprintf("channelpkg.Model %s deleted successfully", req.ModelName),
	})
}

// OllamaVersion 获取 Ollama 服务版本信息
func OllamaVersion(c contract.Context) {
	id, err := strconv.Atoi(c.Param("id"))
	if err != nil {
		_ = c.JSON(http.StatusBadRequest, common.H{
			"success": false,
