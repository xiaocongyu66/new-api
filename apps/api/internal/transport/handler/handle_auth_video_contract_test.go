...
func setupAuthVideoContractTestDB(t *testing.T) {
	...
	taskdomain.GetTaskProviderFunc = func(platform string) taskdomain.TaskProviderExec {
		return nil
	}
...
func createTestUser(t *testing.T, username, password string) *identity.User {
	t.Helper()
	user := &identity.User{
		Username:    username,
		Password:    password,
...
func createTestUserWith2FA(t *testing.T, username, password string) (*identity.User, string) {
	t.Helper()
	user := createTestUser(t, username, password)

	twoFA := &identity.TwoFA{
		UserId:    user.Id,
...
func createTestTask(t *testing.T, userID int, taskID string, channelID int) *taskdomain.Task {
	t.Helper()
	taskModel := &taskdomain.Task{
		...
		ChannelId: channelID,
		PrivateData: taskdomain.TaskPrivateData{
...
func createTestChannel(t *testing.T, channelType int) *channel.Channel {
	t.Helper()
...
func TestVideoProxySuccessPathOpenAIContract(t *testing.T) {
	...
	dbx.DB.Model(taskModel).Update("private_data", taskdomain.TaskPrivateData{
		Key:            "",
