...
func normalizeChannelModelMapping(channel *channelpkg.Channel) map[string]string {
...
func collectPendingUpstreamModelChanges(channel *channelpkg.Channel, settings dto.ChannelOtherSettings) (pendingAddModels []string, pendingRemoveModels []string, err error) {
...
func getFetchModelsResponseBody(method string, requestURL string, channel *channelpkg.Channel, headers http.Header) ([]byte, error) {
...
func fetchChannelUpstreamModelIDs(channel *channelpkg.Channel) ([]string, error) {
	...
	if channel.Type == constant.ChannelTypeCodex {
		return channelpkg.FetchCodexChannelModels(channel)
	}
...
func fetchAdvancedCustomUpstreamModelIDs(channel *channelpkg.Channel, baseURL string) ([]string, error) {
	key, _, apiErr := channel.GetNextEnabledKey("")
	if apiErr != nil {
		return nil, fmt.Errorf("获取渠道密钥失败: %w", apiErr)
	}
...
func updateChannelUpstreamModelSettings(channel *channelpkg.Channel, settings dto.ChannelOtherSettings, updateModels bool) error {
	...
	return dbx.DB.Model(&channelpkg.Channel{}).Where("id = ?", channel.Id).Updates(updates).Error
}
...
func refreshChannelRuntimeCache() {
	if common.MemoryCacheEnabled {
		func() {
			defer func() {
				if r := recover(); r != nil {
					common.SysLog(fmt.Sprintf("channelpkg.InitChannelCache panic: %v", r))
				}
			}()
...
func runChannelUpstreamModelUpdateTaskOnce(ctx context.Context, force bool, allowAutoApply bool, report func(processed, total int)) upstreamModelUpdateSummary {
	...
	if err := dbx.DB.Model(&channelpkg.Channel{}).Where("status = ?", common.ChannelStatusEnabled).Count(&totalChannels).Error; err != nil {
		totalChannels = 0
	}
	processed := 0
...
scanLoop:
	...
	if changedChannels > 0 || failedChannels > 0 {
		...
		ops.NotifyUpstreamModelUpdateWatchers(
			"上游模型巡检通知",
...
func ApplyChannelUpstreamModelUpdates(c contract.Context) {
	...
	channel, err := channelpkg.GetChannelById(req.ID, true)
	if err != nil {
		common.CtxApiError(c, err)
		return
	}
...
func findEnabledChannelsAfterID(lastID int, batchSize int) ([]*channelpkg.Channel, error) {
	var channels []*channelpkg.Channel
	query := dbx.DB.
		Select(channelUpstreamModelUpdateSelectFields).
		Where("status = ?", common.ChannelStatusEnabled).
...
func DetectAllChannelUpstreamModelUpdates(c contract.Context) {
	task, created, err := ops.EnqueueSystemTask(ops.SystemTaskTypeModelUpdate, modelUpdateTaskPayload{Manual: true})
	if err != nil {
		common.CtxApiError(c, err)
		return
	}
