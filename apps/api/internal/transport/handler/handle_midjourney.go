...
// runMidjourneyTaskUpdateOnce performs one taskpkg.Midjourney polling pass synchronously.
...
func runMidjourneyTaskUpdateOnce(ctx context.Context, report func(processed, total int)) midjourneyPollSummary {
	...
	tasks := taskpkg.GetAllUnFinishTasks()
	if len(tasks) == 0 {
		return summary
	}
	...
	taskM := make(map[string]*taskpkg.Midjourney)
	...
	if len(nullTaskIds) > 0 {
		...
		err := taskpkg.MjBulkUpdateByTaskIds(nullTaskIds, map[string]any{
			"status":   "FAILURE",
			"progress": "100%",
		})
		if err != nil {
	...
	for channelId, taskIds := range taskChannelM {
		...
		midjourneyChannel, err := channel.CacheGetChannel(channelId)
		if err != nil {
			logger.LogError(ctx, fmt.Sprintf("channel.CacheGetChannel: %v", err))
			err := taskpkg.MjBulkUpdate(taskIds, map[string]any{
		...
		for _, responseItem := range responseItems {
			...
			if task == nil {
				logger.LogWarn(ctx, fmt.Sprintf("taskpkg.Midjourney task response ignored: unknown mj_id=%s", responseItem.MjId))
				continue
			}
			...
			if err != nil {
				...
				taskdomain.RefundMidjourneyQuota(ctx, task, "构图失败")
			}
		}
	}
...
func checkMjTaskNeedUpdate(oldTask *taskpkg.Midjourney, newTask dto.MidjourneyDto) bool {
	if oldTask.Code != 1 {
		return true
	}
...
func GetAllMidjourney(c contract.Context) {
	...
	// 解析其他查询参数
	queryParams := taskpkg.TaskQueryParams{
