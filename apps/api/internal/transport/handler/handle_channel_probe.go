...
func normalizeChannelTestEndpoint(channel *channelpkg.Channel, modelName, endpointType string) string {
...
func testChannel(ctx context.Context, channel *channelpkg.Channel, testUserID int, testModel string, endpointType string, isStream bool) testResult {
	if ctx == nil {
	...
	if resp != nil {
		...
		if httpResp.StatusCode != http.StatusOK {
			err := relaycommon.RelayErrorHandler(c.Context(), httpResp, true)
	...
	usagedomain.RecordConsumeLog(c, testUserID, usagedomain.RecordConsumeLogParams{
...
func settleTestQuota(info *relaycommon.RelayInfo, priceData hosttypes.PriceData, usage *dto.Usage) (int, *price_expression.TieredResult) {
	if usage != nil && info != nil && info.TieredBillingSnapshot != nil {
		isClaudeUsageSemantic := usage.UsageSemantic == "anthropic" || info.GetFinalRequestRelayFormat() == types.RelayFormatClaude
		usedVars := price_expression.UsedVars(info.TieredBillingSnapshot.ExprString)
...
func buildTestLogOther(c contract.Context, info *relaycommon.RelayInfo, priceData hosttypes.PriceData, usage *dto.Usage, tieredResult *price_expression.TieredResult) map[string]interface{} {
	other := usagedomain.GenerateTextOtherInfo(c, info, priceData.ModelRatio, priceData.GroupRatioInfo.GroupRatio, priceData.CompletionRatio,
		usage.PromptTokensDetails.CachedTokens, priceData.CacheRatio, priceData.ModelPrice, priceData.GroupRatioInfo.GroupSpecialRatio)
	if tieredResult != nil {
		usagedomain.InjectTieredBillingInfo(other, info, tieredResult)
	}
	return other
}
...
func shouldUseStreamForAutomaticChannelTest(channel *channelpkg.Channel) bool {
...
func buildTestRequest(model string, endpointType string, channel *channelpkg.Channel, isStream bool) dto.Request {
...
func TestChannel(c contract.Context) {
	...
	channel, err := channelpkg.CacheGetChannel(channelId)
	if err != nil {
...
func performChannelTests(ctx context.Context, channels []*channelpkg.Channel, testUserID int, allowDisable bool, report func(processed, total int)) channelTestSummary {
	...
	for index, channel := range channels {
		...
		if newAPIError != nil {
			shouldBanChannel = channelpkg.ShouldDisableChannel(result.newAPIError)
		}
		...
		// enable channel
		if result.localErr == nil && !isChannelEnabled && channelpkg.ShouldEnableChannel(newAPIError, channel.Status) {
			channelpkg.EnableChannel(channel.Id, common.GetCtxKeyString(result.context, constant.ContextKeyChannelKey), channel.Name)
			summary.Enabled++
		}
...
func runChannelTestTask(ctx context.Context, mode string, notify bool, report func(processed, total int)) (channelTestSummary, error) {
	...
	channels, err := channelpkg.GetAllChannels(0, 0, true, false)
	if err != nil {
		return channelTestSummary{}, err
	}
	...
	if notify && (ctx == nil || ctx.Err() == nil) {
		ops.NotifyRootUser(dto.NotifyTypeChannelTest, "通道测试完成", "所有通道测试已完成")
	}
	return summary, nil
}

func selectChannelsForAutomaticTest(channels []*channelpkg.Channel, mode string) []*channelpkg.Channel {
	selected := make([]*channelpkg.Channel, 0, len(channels))
...
func TestAllChannels(c contract.Context) {
	task, created, err := ops.EnqueueSystemTask(ops.SystemTaskTypeChannelTest, channelTestTaskPayload{
