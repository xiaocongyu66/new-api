...
func GetChannelModelHealth(c contract.Context) {
	...
	rows, err := channelpkg.ListChannelModelHealth(channelID)
	if err != nil {
	...
	for _, row := range rows {
		...
		} else if state == channelpkg.HealthCalm || state == channelpkg.HealthDormant {
			state = channelpkg.HealthHealthy
		}
...
func UpdateChannelModelHealth(c contract.Context) {
	...
	switch action {
	case "disable":
		err = channelpkg.DisableRoute(key, now)
	case "recover":
		err = channelpkg.RecoverRoute(key, now)
	default:
