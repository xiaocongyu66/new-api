...
func setupManageUserTestDB(t *testing.T) *gorm.DB {
	...
	require.NoError(t, db.AutoMigrate(
		&identity.User{}, &identity.UserSession{}, &usage.Log{}, &policy.CasbinRule{}, &policy.AuthzRole{},
		&identity.QQBinding{}, &identity.QQBindCode{}, &identity.QQUnbindRecord{},
	))
...
func performManageUserRequest(t *testing.T, body string) *httptest.ResponseRecorder {
	...
	c, recorder := fiberadapter.NewSyntheticContext(request)
	c.Set("id", 9999)
...
func TestManageUserDemoteAdvancesAuthVersionAndRevokesSessionsOnce(t *testing.T) {
	...
	assert.EqualValues(t, 2, updated.AuthVersion)
	var sessions []identity.UserSession
...
func TestManageUserDeleteReturnsImmediatelyAndUnknownActionFails(t *testing.T) {
	...
	require.NoError(t, db.Unscoped().Model(&identity.User{}).Where("id = ? AND deleted_at IS NOT NULL", deleted.Id).Count(&deletedCount).Error)
	assert.EqualValues(t, 1, deletedCount)

	unchanged := identity.User{
