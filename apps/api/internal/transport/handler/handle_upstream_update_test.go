...
func newAdvancedCustomModelListChannel(baseURL string, key string, upstreamPath string, auth *dto.AdvancedCustomRouteAuth) *catalogpkg.Channel {
...
func TestFetchAdvancedCustomModelsUsesEnabledSavedMultiKey(t *testing.T) {
	...
	channel.ChannelInfo = catalogpkg.ChannelInfo{
		IsMultiKey: true,
...
func TestFetchModelsAdvancedCustomEditPreviewUsesSavedKeyAndExplicitClears(t *testing.T) {
	...
	savedChannel.ChannelInfo = catalogpkg.ChannelInfo{
		IsMultiKey: true,
		MultiKeyStatusList: map[int]int{
			0: common.ChannelStatusManuallyDisabled,
			1: common.ChannelStatusEnabled,
...
func TestFailedAdvancedCustomDetectionDoesNotStageFullRemoval(t *testing.T) {
	...
	reloaded, err := catalogpkg.GetChannelById(channel.Id, true)
	require.NoError(t, err)
