...
func ensureVendorID(vendorName string, vendorByName map[string]upstreamVendor, vendorIDCache map[string]int, createdVendors *int) int {
	...
	var existing channelpkg.Vendor
...
func SyncUpstreamPreview(c contract.Context) {
	...
	if len(vendorIDs) > 0 {
		var dbVendors []channelpkg.Vendor
	...
	// 3) 缺失且上游存在的模型
	missingList, _ := channelpkg.GetMissingModels()
	var missing []string
