...
func GetCodexChannelUsage(c contract.Context) {
	fetchCodexChannelWhamData(
		c,
		channelpkg.FetchCodexWhamUsage,
...
func GetCodexChannelRateLimitResetCredits(c contract.Context) {
	fetchCodexChannelWhamData(
		c,
		channelpkg.FetchCodexWhamRateLimitResetCredits,
...
func ResetCodexChannelUsage(c contract.Context) {
	fetchCodexChannelWhamData(
		c,
		channelpkg.ConsumeCodexWhamRateLimitResetCredit,
...
func fetchCodexChannelWhamData(
	...
	ch, err := channelpkg.GetChannelById(channelId, true)
	if err != nil {
	...
	if (statusCode == http.StatusUnauthorized || statusCode == http.StatusForbidden) && strings.TrimSpace(oauthKey.RefreshToken) != "" {
		...
		res, refreshErr := channelpkg.RefreshCodexOAuthTokenWithProxy(refreshCtx, oauthKey.RefreshToken, ch.GetSetting().Proxy)
		if refreshErr == nil {
			...
			if encErr == nil {
				_ = dbx.DB.Model(&channelpkg.Channel{}).Where("id = ?", ch.Id).Update("key", string(encoded)).Error
