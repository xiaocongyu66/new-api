...
import (
	...
	constant2 "github.com/QuantumNous/new-api/internal/relay/constant"
...
func EstimateRequestToken(c contract.Context, meta *types.TokenCountMeta, info *RelayInfo) (int, error) {
	// 是否统计token
...
func CountTokenRealtime(info *RelayInfo, request dto.RealtimeEvent, model string) (int, int, error) {
	audioToken := 0
	textToken := 0
	switch request.Type {
