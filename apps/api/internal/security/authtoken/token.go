...
const (
	...
	TokenIssuer           = "new-api"
	TokenAudience         = "new-api-dashboard"
)
...
func SigningKey(purpose string) []byte {
...
func IssueAccessToken(identity AuthIdentity) (string, int64, error) {
	...
	signed, err := jwt.NewWithClaims(jwt.SigningMethodHS256, claims).SignedString(SigningKey(accessTokenUse))
	return signed, expiresAt.Unix(), err
}

func ParseAccessToken(raw string) (AuthIdentity, error) {
	claims, err := parseAuthClaims(raw, accessTokenUse, SigningKey(accessTokenUse))
	if err != nil {
...
func ParseDashboardAccessToken(raw string) (identity AuthIdentity, internal bool, err error) {
	...
	if claims.Issuer != TokenIssuer || !audienceMatches || !knownTokenUse {
		return AuthIdentity{}, false, nil
	}
...
func IssueSecurityProof(identity AuthIdentity, method string, scopes []string) (string, int64, error) {
	...
	signed, err := jwt.NewWithClaims(jwt.SigningMethodHS256, claims).SignedString(SigningKey(securityProofTokenUse))
	return signed, expiresAt.Unix(), err
}

func VerifySecurityProof(raw string, identity AuthIdentity, requiredScope string, allowedMethods []string) (string, error) {
	claims, err := parseAuthClaims(raw, securityProofTokenUse, SigningKey(securityProofTokenUse))
	if err != nil {
		return "", err
	}
	userID, err := strconv.Atoi(claims.Subject)
...
func parseAuthClaims(raw, expectedUse string, key []byte) (*authClaims, error) {
	...
	parsed, err := jwt.ParseWithClaims(raw, claims, func(token *jwt.Token) (any, error) {
		...
		return key, nil
	}, jwt.WithValidMethods([]string{jwt.SigningMethodHS256.Alg()}), jwt.WithIssuer(TokenIssuer), jwt.WithAudience(TokenAudience), jwt.WithExpirationRequired(), jwt.WithIssuedAt(), jwt.WithLeeway(5*time.Second))
	if err != nil {
