...
// SearchPos 一次扫描返回所有命中（词下标 + 结束字节位置），不做词界校验。
// 与 Search 的区别：调用方需要命中位置以便自行做 Unicode 词界判定。
func (m *ByteAC) SearchPos(text string) ([]int32, []int32) {
	if m == nil {
		return nil, nil
	}
	state := 0
	...
	for i := 0; i < len(text); i++ {
		state = int(m.next[state][text[i]])
		if hits := m.output[state]; len(hits) > 0 {
			idx = append(idx, hits...)
			for range hits {
