...
var (
	...
	fpCatOf     []int          // 原子 → 类别下标
	fpAc        *common.ByteAC // 全原子单趟 AC（raw 构建：原子已小写、不裁剪）
)
...
func fingerprintScore(text string) (int, []string) {
	...
	for ci, cat := range fingerprintCategories {
		if catHit[ci] && (cat.group == "" || SensitiveGroupEnabled(cat.group)) {
			score++
